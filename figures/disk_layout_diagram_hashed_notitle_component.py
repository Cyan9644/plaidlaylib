#!/usr/bin/env python3
"""Conceptual diagram: chunk_seq's hashed chunk placement (the "Hashed
Chunking" panel of disk_layout_diagram.py).  Not data-driven (no CSV input,
no CLI sweep) -- it renders synthetic layout data purely to illustrate the
placement strategy.

Variant of disk_layout_diagram_notitle.py's hashed panel: identical diagram,
but the in-memory sub-panel's label reads "In-Memory Component" instead of
"In-Memory Representation" (also still untitled -- no fig.suptitle, per that
variant). Only the hashed panel is reproduced here since the requested label
change is specific to it; the sequential/Peter panel is unaffected and lives
in disk_layout_diagram_notitle.py.
"""
import argparse
...
def build_hashed_figure():
    # No fig.suptitle (see module docstring) -- constrained_layout doesn't
    # reserve title space at all.
...
def main():
    ...
    parser.add_argument(
        "--out-prefix", default="disk_layout_diagram_hashed_notitle_component",
        help="output PNG basename prefix; writes <prefix>.png "
             "(default: %(default)s)")
    args = parser.parse_args()

    out = f"{args.out_prefix}.png"
    fig = build_hashed_figure()
