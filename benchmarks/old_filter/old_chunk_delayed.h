...
// lazy sequence).  For `reduce(map(map(delay(seq),f),g),m)` the eager path
// moves 3n reads + 2n writes; the delayed path moves 1n reads and 0 writes.
//
...
// source (both operands of a `zip`) must outlive every terminal call made on
...
// File-backed: make_iter(const TSrc* raw, size_t n, size_t chunk_index) ->
// iter.
...
// Index-backed: make_iter(size_t base_index, size_t n, size_t chunk_index) ->
// iter.
...
struct delayed_index {
  ...
  size_t num_chunks() const {
    return (n + ELEMS_PER_CHUNK - 1) / ELEMS_PER_CHUNK;
  }
};
...
// have a real chunk for a given index and padding the rest.  make_iter takes
// the (possibly null) raw buffers of both file sides plus the chunk's element
// base, length, index, and each side's real-element count.
...
size_t delayed_length(const D& d) {
  if constexpr (D::is_file) {
    ...
    return d
        .n;  // delayed_index; delayed_zip is never an operand of zip pre-length
  }
}
...
// Lazily map g over every element.  Returns a delayed sequence of the same
// kind; composes with no temp buffer and no I/O.
...
// make_iter, for a chunk, builds each side's inner iterator (from its raw
// buffer if a present file chunk, else generated from the element base for an
// index side, else an unused dummy) wrapped in a pad_iter to the side's real
// count.
  ...
  auto mk = [mkA, mkB, padA, padB](const void* ra, const void* rb, size_t base,
    ...
    auto innerA = [&] {
      if constexpr (DA::is_file) {
        return mkA(reinterpret_cast<const typename DA::source_type*>(ra), rA,
                   ci);
      } else {
        (void)ra;
        return mkA(base, n, ci);
      }
    }();
    auto innerB = [&] {
      if constexpr (DB::is_file) {
        return mkB(reinterpret_cast<const typename DB::source_type*>(rb), rB,
                   ci);
      } else {
        (void)rb;
        return mkB(base, n, ci);
      }
    }();
...
auto zip(DA da, DB db) {
  ...
  return make_zip_seq(da, db, lenA, lenB, typename DA::value_type{},
                      typename DB::value_type{});
}

// Padded zip: if the operands differ in length, the shorter is padded with
// `pad` up to max(lenA, lenB).  A single pad value requires a shared element
// type.
...
auto zip(DA da, DB db, Pad pad) {
  ...
  static_assert(std::is_same_v<VA, VB>,
                ...
                "share a value_type");
  return make_zip_seq(da, db, delayed_length(da), delayed_length(db), (VA)pad,
                      (VB)pad);
}
...
// buffer (nullptr where the side has no chunk).  `reader` must outlive use of
// the buffers; free them via reader.allocator.Free after processing.
template <class T>
...
// grid: read the file sides, then in parallel build each chunk's zipped
// iterator and hand it to per_chunk(local_b, chunk_index, n_elements,
// iterator).
template <class Z, class PerChunk>
void for_zip_batch(const Z& z, size_t base, size_t batch_n,
                   PerChunk&& per_chunk) {
  ...
  if constexpr (DA::is_file && DB::is_file) {
    ...
    for (auto p : pA)
      if (p) rA.allocator.Free(p);
    for (auto p : pB)
      if (p) rB.allocator.Free(p);
...
void run_zip_chunks(const Z& z, size_t /*reader_threads*/, Body&& body) {
  ...
  for (size_t base = 0; base < nc; base += FILTER_BATCH_SIZE) {
    ...
    for_zip_batch(z, base, batch_n, [&](size_t, size_t ci, size_t n, auto it) {
      body(ci, n, it);
    });
  }
}
...
// (file-backed) or zero source I/O (index-backed).
template <class D, class Monoid>
...
auto scan(const D& d, Monoid m) {
  ...
  if constexpr (D::is_zip) {
    auto mk = [inner, m, offsets](const void* ra, const void* rb, size_t base,
                                  size_t n, size_t ci, size_t rA, size_t rB) {
      return make_scan_iter(inner(ra, rb, base, n, ci, rA, rB), m,
                            (*offsets)[ci]);
    };
    return std::pair{
        delayed_zip<typename D::side_a, typename D::side_b, decltype(mk)>{
            d.a, d.b, d.lenA, d.lenB, d.len, mk},
        total};
  } else if constexpr (D::is_file) {
    using TSrc = typename D::source_type;
...
// compaction, prefix sums, parallel scatter, dense CHUNK_SIZE packing — but
// each chunk's elements are produced by walking the delayed iterator, so
// preceding maps fuse into this read pass.  Returns an index-ordered chunk_seq.
...
chunk_seq filter(const D& d, const std::string& result_prefix, Pred pred) {
  ...
  for (size_t base = 0; base < n_in; base += FILTER_BATCH_SIZE) {
    ...
    for (size_t k = 0; k < num_alloc; k++) {
      ...
      CHECK(obuf[k] != nullptr)
          << "old_delayed::filter: output allocation failed";
