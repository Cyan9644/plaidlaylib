...
static size_t k_for(size_t n, const std::string& mode, size_t k0) {
    ...
    if (mode == "log")    return k0 * (size_t)std::ceil(std::log2((double)std::max<size_t>(n, 2)));
...
static bool verify_chunk0(const chunk_seq& out, uint64_t k) {
    ...
    if (fd < 0) {
        std::cerr << "  FAIL chunk 0: open(" << c.filename << "): " << strerror(errno) << "\n";
        return false;
    }
    const size_t read_size = AlignUp(c.used);
    uint64_t* buf = (uint64_t*)aligned_alloc(O_DIRECT_MEMORY_ALIGNMENT, read_size);
    CHECK(buf != nullptr);
    const ssize_t got = pread(fd, buf, read_size, (off_t)c.begin_addr);
    close(fd);
    if (got < 0 || (size_t)got < c.used) {
        std::cerr << "  FAIL chunk 0: pread got " << got << " expected at least " << c.used << "\n";
        free(buf);
        return false;
    }
    ...
    for (size_t i = 0; i < count && ok; i++) {
        const uint64_t expected = i ^ churn(i, k);   // input chunk 0 holds values [0, count)
        if (buf[i] != expected) {
...
int main(int argc, char* argv[]) {
    ...
    if (mode == "linear") {
        ...
        if (const char* e = getenv("WORK_EXPONENT_MAX_TOTAL_OPS")) max_total_ops = std::stoull(e);
    ...
    if (const char* e = getenv("WORK_EXPONENT_INMEM_BUDGET_BYTES")) budget = std::stoull(e);
    ...
    if (axis == "elemwork") {
        ...
        for (size_t r = 0; r < std::max<size_t>(R, 1); r++) {
            ...
            if (r == 0) first_round = s;
            else if (s != first_round) rounds_agree = false;
    ...
    std::cout << (agree ? "agree=1 (all checks passed)" : "agree=0 (MISMATCH -- see above)")
              << "   op_s=" << std::fixed << std::setprecision(4) << op_s
              << "   " << std::setprecision(2) << throughput_gb_s << " GB/s (eff. input)\n";

    auto f9 = [](double v) { std::ostringstream o; o << std::setprecision(9) << v; return o.str(); };
    std::cout << "CSV," << axis << ',' << mode << ',' << n << ',' << k0 << ',' << k_effective
              << ',' << total_ops << ',' << f9(build_s) << ',' << f9(op_s)
              << ',' << f9(throughput_gb_s) << ',' << (agree ? 1 : 0) << '\n';
    return agree ? 0 : 1;
}
