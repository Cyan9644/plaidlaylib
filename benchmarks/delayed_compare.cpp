...
//   raw read         ChunkReduce(iota)                          (device-read
//   ceiling) map|reduce       eager 2r+1w (3n)  delayed 1r (n)  in-mem
//   delayed::reduce(map) map|map|reduce   eager 3r+2w (5n)  delayed 1r (n)
//   in-mem delayed::reduce(map(map)) force(map|map)   eager 2r+2w (4n)  delayed
//   1r+1w (2n)  in-mem delayed::to_sequence(map(map))
//
...
// agree=0 and the program exits non-zero.  Eager intermediates are written
// under bw_dl_* and cleaned up after each step.
//
...
static void print_row(const std::string& label, size_t in_bytes, double secs) {
  std::cout << "  " << std::left << std::setw(16) << label << std::right
            << std::fixed << std::setprecision(4) << std::setw(9) << secs
            << "s   " << std::setprecision(2) << to_gb(in_bytes) / secs
            << " GB/s (eff. input)\n";
}
...
int main(int argc, char* argv[]) {
  ...
  if (const char* e = getenv("DELAYED_INMEM_BUDGET_BYTES"))
    budget = std::stoull(e);
  const size_t in_bytes_logical = n * sizeof(uint64_t);
  const bool inmem_reduce_ok = in_bytes_logical <= budget;  // ~8n resident
  const bool inmem_force_ok =
      in_bytes_logical * 2 <= budget;  // ~16n (input + output)

  std::cout << "n=" << n << "   RAM budget for in-memory baseline "
            << std::fixed << std::setprecision(2) << to_gb(budget)
            << " GB (phys " << to_gb(phys) << " GB)\n"
            << "in-mem reduce lines: "
            << (inmem_reduce_ok ? "ON" : "OFF (n exceeds budget)")
  ...
  double raw_s = 0, e_mr = 0, d_mr = 0, i_mr = 0, e_mmr = 0, d_mmr = 0,
         i_mmr = 0, e_f = 0, d_f = 0, i_f = 0;
  bool agree = true;
  ...
  std::cout << "\n--- map(x+1) | reduce(sum) ---\n";
  {
    ...
    if (inmem_reduce_ok) {
      auto t2 = Clock::now();
      uint64_t mr_i =
          parlay::delayed::reduce(parlay::delayed::map(A, add1), psum);
      i_mr = elapsed(t2);
  ...
  // Time only the materialization; the verifying reduce of each output is
  // untimed.
  std::cout << "\n--- force(map(x+1) | map(2x)) — write-terminated ---\n";
  {
    auto t0 = Clock::now();
  ...
  std::cout << "CSV," << n << ',' << f9(raw_s) << ',' << f9(e_mr) << ','
            << f9(d_mr) << ',' << opt(inmem_reduce_ok, i_mr) << ',' << f9(e_mmr)
            << ',' << f9(d_mmr) << ',' << opt(inmem_reduce_ok, i_mmr) << ','
