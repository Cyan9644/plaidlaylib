...
auto build_chain(const std::vector<chunk_seq>& srcs) {
  if constexpr (K == 1) {
    ...
    return cd::map(
        cd::zip(prev, cd::delay<uint64_t>(srcs[K - 1])),
...
void run_depth(const std::vector<chunk_seq>& srcs, size_t n, size_t max_k) {
  ...
  if (!ok) {
    std::cout << "  *** MISMATCH at k=" << K << ": got " << r << " expected "
              << expected << " ***\n";
    g_agree = false;
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Generating " << max_k << " source chunk_seqs of n=" << n
            << " each...\n";
  std::vector<chunk_seq> srcs(max_k);
  for (size_t i = 0; i < max_k; i++)
    srcs[i] = plaid::tabulate<uint64_t>(n, "zsrc" + std::to_string(i),
                                        [](size_t j) { return (uint64_t)j; });
