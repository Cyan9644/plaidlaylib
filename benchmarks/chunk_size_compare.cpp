...
//   (1n I/O) map|reduce        eager 2r+1w (3n)  delayed 1r (1n) map|map|reduce
//   eager 3r+2w (5n)  delayed 1r (1n)  — clearest fusion win force(map|map)
//   eager 2r+2w (4n)  delayed 1r+1w (2n)
//
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "CHUNK_SIZE=" << CHUNK_SIZE << " bytes (" << CHUNK_SIZE / 1024
            << " KB)"
            << "  n=" << n << "\n";
  ...
  std::cout << "CSV," << CHUNK_SIZE << ',' << n << ',' << f9(raw_s) << ','
            << f9(e_mr) << ',' << f9(d_mr) << ',' << f9(e_mmr) << ','
            << f9(d_mmr) << ',' << f9(e_f) << ',' << f9(d_f) << ','
            << (agree ? 1 : 0) << '\n';
  return agree ? 0 : 1;
}
