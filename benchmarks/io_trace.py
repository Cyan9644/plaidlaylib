...
def plot_trace(ser, markers, devices, t0, path, hide_kinds=frozenset(), title_suffix=""):
    ...
    ax_bw.plot(xs, ser["agg_read"], "-", color=plot_style.PALETTE["blue"], label="read", linewidth=1.3)
    ...
    ax_bn.plot(xs, ser["mean_util"], "-", color=plot_style.PALETTE["violet"], label="mean drive %util", linewidth=1.3)
    ax_bn.plot(xs, ser["cpu"], "-", color=plot_style.PALETTE["green"], label="CPU %", linewidth=1.3)
    ax_bn.plot(xs, ser["iowait"], "-", color=plot_style.PALETTE["orange"], label="iowait %", linewidth=1.3)
