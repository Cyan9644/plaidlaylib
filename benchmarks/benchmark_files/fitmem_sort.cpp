// Example: out-of-core fit-in-memory sample sort via
// plaid::fitmem_sort.
//
...
// materialized and sorted directly in DRAM (assumed to fit) rather than
// recursed on out of core.  Builds an n-element sequence of pseudo-random
// uint64 keys across the SSDs (deterministic from parlay::hash64, so
// reproducible and duplicate-free), then sorts them.
//
...
// the two substrates must agree exactly.  Budget: half of physical RAM,
// override via EXAMPLE_INMEM_BUDGET_BYTES; when skipped the CSV field is left
// blank so the plotted in-mem line stops at the RAM cliff (as in the other
// examples).
//
...
// two names coexist.
#include "benchmarks/benchmark_files/fitmem_sort.h"
...
int main(int argc, char* argv[]) {
  ...
  t0 = Clock::now();
  chunk_seq sorted = plaid::fitmem_sort<uint64_t>(seq);
