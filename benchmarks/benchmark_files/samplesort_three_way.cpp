...
// substrate*: same algorithm, same I/O layer, different data model, so the gap
// is what the chunk_seq representation costs.  (2) vs (3) isolates *the
// primitives*: same data model, same algorithm, so the gap is what the
// library's generality costs over hand-written I/O.  The two existing pairwise
// sweeps (external_samplesort_vs_peter, direct_samplesort_vs_peter) each
// measure one of those gaps against a *separately timed* Peter run; this driver
...
// between:
//   Every sort writes its own input, its intermediates and its output, so
//   running three of them back to back would have the later ones sorting on
//   drives the earlier ones left dirty.  Each sort is therefore timed on drives
//   that hold nothing but its own freshly written input, and — the part that
//   actually matters — the teardown between sorts *waits for the file system to
//   finish* with the previous one: settle_drives() syncs every mount and then
//   lets it sit. Without that wait, unlinking a sort's files returns long
//   before ext4 has committed the transactions that free their blocks, and the
//   next sort queues behind that background work: measured at 15-25% on the dev
//   box, which is more than the gaps this benchmark exists to measure.  With
//   it, a sort runs at the same speed wherever it sits in the order (SS3_FIRST
//   lets you verify that: rotating the order must not move the times).
//
//   Repeating each sort and taking the best would also have hidden the ordering
//   bias, but at 3x the bytes written per point — these are consumer SSDs with
//   a finite write budget, so a point costs exactly one sort per
//   implementation.
//
// Caveat on the small end of a sweep: all three sorts derive their pivot count
// as input_bytes / 128 MB, and Peter's GetPivots underflows when that is <= 3
// (see deviation 2 in direct_samplesort.h) — it reads past the sample array and
// takes a garbage pivot, which leaves his buckets unbalanced (sometimes empty).
// His output is still correctly sorted, but *his* series is not meaningful
// below ~512 MB of input; ours are (our port clamps the same loop).
//
//   usage: samplesort_three_wayExample [global --flags] [n]
//     n                number of keys (default 1e6; must be a multiple of 512)
//     BENCH_SETTLE_MS  how long the drives must sit idle after a sync, both
//     after
...
//   throughput is input bytes (n*8) over the sort's own time.  <inmem_s> is
//   left BLANK past the RAM budget, so the plotted DRAM line stops at the
//   cliff.
//
// Why a shim: Peter's headers ship their own configs.h / utils/file_utils.h
// that clash by include guard with the main repo's, so they cannot share a TU
// with the chunk_seq code.  peter_shim.{h,cpp} isolate his sort behind a
// plain-typed interface; this driver never includes his headers.
//
...
#include "ChunkSequence/examples/external_TODO/peter_samplesort/peter_shim.h"
...
// (for both of ours the sorted result *is* a set of files on the drives, so
// those prefixes must be swept or the whole output leaks every sweep point).
//   Peter's:     pss_in / pss_out + his hard-coded spfx_ buckets.
//   direct:      dss_in + dss<tag>_tmp<b> (buckets) and dss<tag>_<b> (the
//   result). primitives:  ss_in + the recursion's
//   ss_id_/ss_bucket_/ss_base_/ss_deg_ and
//                the per-bucket base sorter's qs_base_ output (what flatten
//                returns).
static const std::vector<std::string> kPeterPrefixes = {"pss_in", "pss_out",
                                                        "spfx_"};
...
int main(int argc, char* argv[]) {
  ...
  CHECK(n % 512 == 0)
      << "n must be a multiple of 512 (O_DIRECT alignment); got " << n
  ...
  // out-of-core output materialized at a time to compare against it (8n) —
  // ~24n, the same gate the other examples use.  Past it, the three disk sorts
  // still run and are timed; the in-mem series and the contents check are
  // skipped (the CSV field is left blank, so the plotted DRAM line stops at the
  // RAM cliff).
  const size_t phys =
  ...
  sorters[0].read_back = [&] {
    return peter_shim::ReadBackSorted(pss_files, pss_sizes);
  };
  ...
  // Start from drives clear of every sort's files (including a stale run's),
  // and clear of the freeing work that removing them just queued.
