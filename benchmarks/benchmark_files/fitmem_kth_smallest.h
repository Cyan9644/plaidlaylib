...
T fitmem_kth_smallest(chunk_seq& seq, long k, Less less1 = {}) {
  ...
  //   auto seconds = pivots | std::views::elements<1>; //heap tree needs a
  //   random access container so we can't use views
  ...
  // passes rather than materializing a bucket-id chunk_seq to disk with
  // ChunkMap (which cost a write of the ids plus a read-back in each of the two
  // uses).
  ...
  // is O(n/sqrt(m)) for m buckets), so instead of recursing out-of-core we pull
  // it in and select directly, adjusting k to the bucket-local rank (k -
  // offsets[id]). parlay::kth_smallest returns an *iterator* to the selected
