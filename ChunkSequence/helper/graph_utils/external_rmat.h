...
// (Primitives/sort.h), which for a program-defined T with no numeric_limits
// specialization is a value-initialized T -- the *minimum*, not the maximum --
// so it would silently misroute.  direct_sample_sort needs only operator< /
// operator==.
...
// dense_pack.h has since been merged into chunk_seq.h (DensePackStream).
#include "ChunkSequence/Primitives/chunk_seq.h"
#include "ChunkSequence/examples/direct_samplesort.h"
...
// Remove every file on every drive whose name begins with `prefix` (sort
// leaves tag-suffixed intermediates a GetFileName enumeration would miss).
// Was plaid::sa_detail::sweep (ChunkSequence/examples/external_TODO/
// chunk_sa_common.h, not otherwise recovered here) -- reimplemented locally
// rather than pulling in that whole (unrelated) suffix-array header.
