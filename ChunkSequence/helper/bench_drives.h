...
// samplesort_three_way).
//
// Those drivers time several out-of-core sorts in one process, on one set of
// drives.  Everything here runs strictly *between* the timed regions: it
// decides what state the drives are in when a sort starts, and never touches
// the sort itself.
//
// The subtle one is settle_drives().  A sort that runs after another was
// measured 15-25% slower on the dev box purely because of what the previous one
// left behind, and a bare sync()+sleep does not fix it:
//
//   - unlink() returns as soon as the directory entry is gone.  ext4 releases
//   the
//     file's blocks when the journal transaction commits, in the background —
//     and a sort's files are gigabytes.  A sort started on top of that freeing
//     queues behind it.
...
// (consumer SSDs keep doing GC/TRIM work after the kernel is done).  With this
// in place a sort runs at the same speed wherever it sits in the order, so each
// sort can be timed ONCE per data point — repeating a point and keeping the
// best would hide the bias at the price of multiplying the SSD write endurance
// it costs.
//
...
// them: the file systems are synced (which completes both writeback and the
...
// then settle.  A generic scan (not GetFileName(prefix, d)) because the sorts
// leave tag- and bucket-suffixed intermediates that a fixed 0..num_drives
// enumeration would miss.  This is how each sort is guaranteed to run on drives
