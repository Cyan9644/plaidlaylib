...
#include "ChunkSequence/Primitives/cut.h"
...
struct chunk_csr {
  ...
  chunk_seq
      edges;  // this was originally two sequences but I think it's better to

  // void set_scan_adj_matrix(T graph){//let's assume that we're accepting a
  // graph adjacency matrix that implements some interface that gives us access
  ...
  // now the big question is how we actually get a compressed sparse row graph
  // in the first place. considering that we have the edges with vertices and
  // weights stored on disk
  // in the actual struct, I think the best way to do this is a streaming
  // approach that reads edges from the given file and reconstructs the sparse
  // graph the cleanest way to do this would be in this format:

  // v1 __ v2 __ edge_weight

  // so ideally we would quickly compute a sum of all vertices by reading the
  // file, construct the prefix scan vector in memory, and then set up a second
  // phase where the edges are placed into position. This would be easy with a
  // more basic sequence representation. I think the best approach is to give
  // each vector a series of chunk headers and fill them in, but this gives us a
  // bunch of partially filled chunks. Ideally these should be compressed to
  // form one overall chunk sequence for the edge list

  // it would perhaps be nice to have a version of this method that reads from
  // multiple files, but since a single file can exist across multiple disks (I
  // think), maybe it's not necessary
  ...
  // //compute counts and a prefix scan over the file; obviously processing the
  // file sequentially is kind of slow, so we'll want to think of a better way
  // to do this
...
//     // void set_scan_adj_matrix(T graph){//let's assume that we're accepting
//     a graph adjacency matrix that implements some interface that gives us
//     access to the degree of each node
//     //     //any practical graph is probably stored in a file
//     //     for(size_t i : grp)
//     // }

//     //this method should accept a vertex ID n and return a parlay sequence of
//     the vertices connected to n parlay::sequence<size_t> get_adjacent(size_t
//     n){
...
//     //one thing that might be useful is conversion to compressed sparse
//     column format
