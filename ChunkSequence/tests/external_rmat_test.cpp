...
//   - every edge, element-wise: destination vertex AND exact long double weight,
...
bool check_dense(const chunk_seq& seq, size_t elem_size, size_t expected_elems) {
    ...
    if (total != expected_elems) {
        std::cout << "  FAIL: edge chunks hold " << total << " elements, degree_scan says "
                  << expected_elems << "\n";
        return false;
    }
    return true;
...
bool run_case(size_t n_req, size_t avg_degree, const std::string& label) {
    ...
    if (graph.degree_scan[0] != 0) {
        std::cout << "  FAIL: degree_scan[0] = " << graph.degree_scan[0] << ", expected 0\n";
        ok = false;
    }
    ...
    if (WG.size() != n) {
        std::cout << "  FAIL: in-memory graph has " << WG.size() << " vertices, expected "
                  << n << "\n";
        ok = false;
    } else {
        size_t m_ref = 0;
        ...
        if (m_ref != m) {
            std::cout << "  FAIL: in-memory graph has " << m_ref << " edges, out-of-core has "
                      << m << "\n";
            ok = false;
        }
    }
    ...
    if (ok) {
        ...
        CHECK(edges.size() >= m) << "materialize returned " << edges.size()
                                 << " < " << m << " edges";
        for (size_t v = 0; v < n && ok; v++) {
            std::vector<std::pair<size_t, long double>> ref(WG[v].begin(), WG[v].end());
            ...
            for (size_t k = 0; k < deg; k++) {
                ...
                if (e.connecting_vertex != ref[k].first || e.edge_weight != ref[k].second) {
