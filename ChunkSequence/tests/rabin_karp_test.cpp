...
// hashing under test) that regenerates the text char-by-char from f(i), so
// even multi-batch cases need no O(n) text buffer.  Returns all (possibly
// overlapping) match-start positions in order.
static std::vector<uint64_t> reference_search(
...
// Builds a char text of length n via tabulate<char>(f), runs ChunkRabinKarp
// with `pat`, and verifies count / packing / index order / exact positions
// against the streaming reference.  Cleans up all files, returns true iff PASS.
static bool run_rk_test(const std::string& name, size_t n,
  ...
  chunk_seq matches =
      ChunkSequenceOps::ChunkRabinKarp<char>(text, out_prefix, pat);
      ...
      while (ok) {
        ...
        for (size_t i = 0; i < count && ok; i++, j++) {
          if (j >= expected.size() || buf[i] != expected[j]) {
            std::cout << "    FAIL position: element " << j << " got " << buf[i]
                      << " expected "
                      << (j < expected.size() ? std::to_string(expected[j])
                                              : std::string("<none>"))
                      << "\n";
            pass = ok = false;
          }
...
int main(int argc, char* argv[]) {
  ...
  // ── 1. Random text, short pattern, partial last chunk ────────────────────
  all_pass &=
      run_rk_test("random_abab", 3 * CHARS_PER_CHUNK + 7, random4, "abab");
  ...
  // (and every window hash is a hit, so the double-check runs constantly).
  all_pass &= run_rk_test(
      "all_a_dense", 2 * CHARS_PER_CHUNK + 5, [](size_t) { return 'a'; },
      "aaaa");

  // ── 3. No matches: pattern contains a letter outside the alphabet ────────
  all_pass &= run_rk_test("no_match", CHARS_PER_CHUNK + 3, random4, "abz");
  ...
  // and flush against the end of the text.
  {
    ...
    all_pass &= run_rk_test("planted_boundary_and_end", n, f, pat);
  }
  ...
  // synchronous seam-head read.  argv[1] overrides n (min 2 chunks).
  {
    ...
    all_pass &= run_rk_test("multi_batch_seam", n, f, pat);
  }
