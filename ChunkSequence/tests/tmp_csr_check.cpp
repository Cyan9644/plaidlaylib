...
int main(int argc, char* argv[]) {
  ...
  check(g.degree_scan.size() == 5,
        "degree_scan size (expected 5 for 4 vertices)");
  ...
  for (auto& e : adj0) {
    if (e.connecting_vertex == 1) {
      check(e.edge_weight == (weight)1.5, "0->1 weight");
      has1 = true;
    }
    if (e.connecting_vertex == 2) {
      check(e.edge_weight == (weight)2.5, "0->2 weight");
      has2 = true;
    }
  }
