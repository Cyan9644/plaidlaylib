...
// exercise of the delayed layer (padded zip, nested zip, scan carry across
// chunk boundaries).
//
...
// the public contract (b sign-extended to a's length; one extra sign limb on
...
static std::vector<digit> ref_add(std::vector<digit> a, std::vector<digit> b,
  ...
  for (size_t i = 0; i < na; i++) {
    unsigned __int128 s =
        (unsigned __int128)a[i] + (i < nb ? b[i] : bpad) + carry;
    r[i] = (digit)s;
    carry = s >> 64;
  }
...
static void check(const std::string& name, const std::vector<digit>& a,
  ...
  cleanup_prefix("bta_a");
  cleanup_prefix("bta_b");
  cleanup_prefix("bta_eager");
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Big-integer add test  (" << GetSSDList().size()
            << " drives)\n\n";
  ...
  check("2^62 + 2^62 -> append 0 (pos overflow)", {(digit)1 << 62},
        {(digit)1 << 62});  // {2^63, 0}
  check("(-2^63) + (-2^63) -> append ~0 (neg overflow)", {(digit)1 << 63},
        {(digit)1 << 63});  // {0, ~0}
  ...
  for (size_t n : {E, 2 * E + 3})
    check("all-ones + 0 + 1  n=" + std::to_string(n), filled(n, MAX),
          filled(n, 0), true);
  ...
  for (size_t n : {size_t(1), size_t(9), E - 1, E, E + 1, 2 * E + 17}) {
    check("random  n=" + std::to_string(n), random_limbs(n, 100 + n),
          random_limbs(n, 900 + n));
  }
