...
//      with the pure in-memory Kärkkäinen–Sanders base case (budget forced
//      huge) on a text spanning several chunks.
//
// Every tiny case is run under BOTH budgets, so both the streaming recursion
// and the DRAM base case are checked against brute force.  Exits 0 iff all
// pass.
...
int main(int argc, char* argv[]) {
  ...
  if (failures == 0) {
    const size_t n =
        (argc > 1) ? std::stoull(argv[1]) : 5'000'000;  // > 1 chunk
    std::cout << "multi-chunk differential n=" << n << " ..." << std::flush;
    std::string s(n, 'a');
    for (auto& ch : s) ch = (char)('a' + rng() % 4);
    const std::vector<uint32_t> a = dc3_of(s, "0");  // full out-of-core
    const std::vector<uint32_t> b =
        dc3_of(s, "1000000000000");  // in-memory base
    if (!eq(a, b)) {
      std::cout << " FAIL (streaming vs DRAM disagree)\n";
      failures++;
    } else
      std::cout << " ok\n";
  }
  ...
  if (failures == 0)
    std::cout << "dc3Test: ALL PASS\n";
  else
    std::cout << "dc3Test: " << failures << " FAILURES\n";
