...
// with a small DRAM budget, so multi-chunk operands actually recurse through
// the out-of-core cut/shift/add path rather than falling straight into the DRAM
// base case.  Exit 0 on all-pass, 1 otherwise.
...
static std::vector<digit> materialize(const chunk_seq& seq) {
  ...
  while (true) {
    ssize_t got = plaid::io::Read(fd, buf.data(), buf.size() * sizeof(digit));
    CHECK(got >= 0);
    if (got == 0) break;
...
static void trim(std::vector<digit>& a) {
...
static std::vector<digit> school(const std::vector<digit>& a,
...
static std::vector<digit> ref_mul(const std::vector<digit>& a,
                                  const std::vector<digit>& b) {
  auto [sa, ma] = to_mag(a);
  auto [sb, mb] = to_mag(b);
  ...
  u128 carry = 1;
  for (size_t i = 0; i < p.size() && carry; i++) {
    u128 t = (u128)p[i] + carry;
    p[i] = (digit)t;
    carry = t >> 64;
  }
  return p;
...
static void check(const std::string& name, const std::vector<digit>& a,
  ...
  cleanup_prefix("bmt_a");
  cleanup_prefix("bmt_b");
  cleanup_prefix("bmt_out");
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Big-integer mul test  (" << GetSSDList().size()
            << " drives, CHUNK_SIZE=" << CHUNK_SIZE << ")\n\n";

  const size_t E = ELEMS_PER_CHUNK;
  // Small budget so anything above ~2 chunks recurses through the out-of-core
  // path.
  ...
  for (size_t n : {E - 1, E, E + 1}) {
    check("nonneg n=" + std::to_string(n), random_nonneg(n, 100 + n),
          random_nonneg(n, 900 + n));
  }

  std::cout << "  out-of-core recursion (multi-chunk, small budget)\n";
  for (size_t n : {2 * E + 3, 3 * E, 5 * E + 7}) {
    check("nonneg n=" + std::to_string(n), random_nonneg(n, 11 + n),
          random_nonneg(n, 77 + n));
    check("signed  n=" + std::to_string(n), random_limbs(n, 22 + n),
          random_limbs(n, 88 + n));
  }

  std::cout << "  unequal lengths (unbalanced split branch)\n";
  check("nonneg 6E x 2E", random_nonneg(6 * E, 41), random_nonneg(2 * E, 42));
  check("nonneg 5E x 1 (tiny b)", random_nonneg(5 * E, 43),
        random_nonneg(1, 44));
