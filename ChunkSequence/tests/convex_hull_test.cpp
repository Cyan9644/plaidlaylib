...
// short-circuiting into the in-memory base case), and checks the result two
// ways:
//
//   1. Differential: it must be byte-for-byte the same hull (same vertex
//   indices,
//      same left-to-right order) as parlaylib's in-DRAM upper_hull on the same
//      points.  This pits the whole out-of-core machinery against an
...
//      hull edge (so it really is the upper boundary) — a check that would
//      catch a bug shared by both quickhull implementations.
//
...
static bool geometry_ok(const std::vector<hpoint>& pts,
  ...
  if (H.front().idx != mn.idx || H.back().idx != mx.idx) {
    std::cout
        << "    FAIL geometry: hull endpoints are not the (x,y) extremes\n";
    return false;
  }
...
static bool check_hull(const std::string& label, const std::vector<hpoint>& pts,
  ...
  if (hull.size() != hull_mem.size()) {
    ...
    if (pass)
      std::cout << "    " << label << " match  OK (" << hull.size()
                << " vertices)\n";
  }

  // 2. Independent geometry.
  if (pass && !geometry_ok(pts, hull))
    pass = false;
  else if (pass)
    std::cout << "    " << label << " geometry OK\n";

  // 3. The recursion actually left DRAM when we expected it to.
  if (expect_ext_recursion && splits == 0) {
    std::cout << "    FAIL " << label
              << ": expected out-of-core recursion but none occurred\n";
    pass = false;
  } else {
...
static bool run_case(const std::string& name, size_t n,
  ...
  pointseq Pts =
      parlay::tabulate(n, [&](size_t i) { return point{pts[i].x, pts[i].y}; });
...
int main(int argc, char* argv[]) {
  ...
  // Small hand case: a square with interior points (hull = the two top
  // corners).
  ok &= run_case(
      "small square + interior", 6,
      [](size_t i) {
