// in_memory_baselines.h -- the DRAM reference implementations the examples
// cross-check themselves against, for the two cases with no upstream parlaylib
// equivalent: linefit and sample sort.  The other five examples baseline
// against deps/parlaylib-examples/ directly.

#ifndef PLAID_IN_MEMORY_BASELINES_H
#define PLAID_IN_MEMORY_BASELINES_H
...
// ============================================================================
// in-memory least-squares line fit
//
// (was ChunkSequence/examples/in_memory_baselines.h)
// ============================================================================
...
// Upstream includes this as "helper/heap_tree.h" relative to itself; from our
// tree the parlaylib-examples checkout is reached through the -Ideps root.
...
void sample_sort_(Range in, Range out, Less less, int level = 1) {
  ...
  int bits =
      std::min<long>(8, parlay::log2_up(n) - parlay::log2_up(cutoff) + 1);
  ...
  // put pivots into efficient search tree and find buckets id for the input
  // keys
  heap_tree ss(pivots);
  auto bucket_ids = parlay::tabulate(
      n, [&](long i) -> unsigned char { return ss.find(in[i], less); });

  // sort into the buckets
  auto [keys, offsets] =
      parlay::internal::count_sort(in, bucket_ids, num_buckets);

  // now recursively sort each bucket
  parlay::parallel_for(
      0, num_buckets,
      [&, &keys = keys, &offsets = offsets](long i) {
        ...
        if (i == 0 || i == num_buckets - 1 || less(pivots[i - 1], pivots[i]))
          sample_sort_(keys.cut(first, last), out.cut(first, last), less,
                       level + 1);
        else
          parlay::copy(keys.cut(first, last), out.cut(first, last));
      },
      1);
}
...
#endif  // PLAID_IN_MEMORY_BASELINES_H
