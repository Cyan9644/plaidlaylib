...
//                    intermediate writes.
//   * eager_add_s  — ChunkBigIntAddEager (non-delayed): the same computation
//   but
//                    with the intermediate map(classify) and scan(carry)
//                    results materialized to disk between primitives, so the
//                    plot shows the I/O cost the delayed layer avoids.
...
// always ends with a machine-readable `CSV,` line that
// benchmarks/run_benches.py greps. The eager output is cross-checked
// limb-for-limb against the fused output (and, when it runs, the in-mem
// baseline); any mismatch exits non-zero.
//
// Usage: bigint_add_eagerExample [global --flags] [n]   (n = number of 64-bit
// limbs)
...
// Stream-compare two out-of-core chunk_seqs (both index-ordered, produced by
// the same force + overflow push_back, so identical chunk layout) one chunk at
// a time off disk.  Uses O(CHUNK_SIZE) RAM — unlike materializing either output
// into a DRAM vector, which is ~8*n bytes and OOMs at benchmark scale (the
// whole point of the out-of-core path is that n exceeds RAM).
...
int main(int argc, char* argv[]) {
  ...
  const bool inmem_ok =
      n <= budget / (4 * sizeof(digit)) && input_bytes <= INMEM_INPUT_CAP;
  ...
  std::cout << result_limbs << " result limb(s)   fused "
            << std::setprecision(4) << add_s << "s   eager " << eager_add_s
            << "s   " << std::setprecision(2) << gb_s
            << " GB/s (fused, operands read)\n";
  ...
  // Columns:
  // n,build_s,add_s,eager_add_s,inmem_add_s,result_limbs,throughput_gb_s
