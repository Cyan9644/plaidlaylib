...
chunk_seq sample_sort(chunk_seq& seq, Less less1 = {}, size_t disk_span = 1) {
  ...
  if (n < num_samples) {
    // we're likely going to want a fast quicksort method that takes better
    // advantage of our memory representation than just
    // external->materialize->sort->external but this clearly should work so
    ...
    _pt.mark("base:sort");
    // this is going to be really slow if we just materialize and write back,
    // probably the best thing to do is to pass a parlay sequence at the
  ...
  auto pivots = ChunkSequenceOps::sample<T>(seq, sample_size * over);
  ...
  // ChunkSequenceOps::count_sort<T>(seq, ids, externalSequenceVector,
  // "ss_bucket_" + tag);
  ...
  // //one potential optimization here is that ChunkMap doesn't really need to
  // write its data back to disk
  // //we could just make this delayed
  // //the issue then is that count_sort takes a materialized chunk sequence, so
  // we'll need to edit it or make a new one
  ...
  std::vector<chunk_seq> externalSequenceVector(num_buckets);
  // ChunkSequenceOps::count_sort(ids, num_buckets,
  ...
  // the individual external sequences in this case we just need a simple
  // flatten to put all the chunk headers into a single list because each
  // individual sequence should be sorted and they're in order, we'll get a
  // total sorted ordering
  ...
  //     }
  //     if (z ==n) {
  //         //if we're not going to get anything from the partition, i.e. we
  //         have an empty partition auto v =
  ...
  //     //recurring on this samplesort chain is pretty expensive due to the
  //     buffer allocation
  //     //we might consider just an external quicksort for this level, as it
  //     would use much less memory.
  //     //probably multiple levels of parallelism won't help much regardless
  //     since we need to do reads
  //     //for the pivots = large overhead on recurring calls

  //     //so instead we're going to call the external quicksort method, which
  //     we actually have a method for now but it's
  //     //implemented using primitives.
  //     //to get the best performance out of this example, we're going to want
  //     to implement it manually with a reader/writer externalSequenceVector[i]
  ...
  // the basic paradigm here in standard samplesort is that you should have the
  // offset count from the counting sort this gives you the logical indices for
  // the first and last portions of the array you need to sort in parallel
  // obviously this relies on you being able to index into the array
  // efficiently, which implies we're working in DRAM

  // Ok so it seems like this is going to take a very long time -- we need both
  // the chunkmap of heap positions and the the original sequence

  // sadly the parlay::internal::count_sort is difficult here -- in the parlay
  // example, they rely on it to reorder the sequence such that all the
  // 0-bucketed elements stably are arranged One thing that we should note here
  // is that physically moving data is really going to be terrible whatever we
  // do, it needs to move data virtually
...
chunk_seq sample_sort_random(chunk_seq& seq, Less less1 = {}, size_t disk_span = 1) {
  ...
  parlay::sequence<T> seconds(heap_size, std::numeric_limits<T>::max());
  parlay::parallel_for(0, sample_size,
                       [&](size_t i) { seconds[i] = pivots[i].second; });
  parlay::internal::heap_tree ss(seconds);
  _pt.mark("sample:pivots/heap");
...
// samplesort implementation with primitives
template <typename T, typename Less = std::less<>>
chunk_seq sample_sort_singledrive(chunk_seq& seq, Less less1 = {}) {
  static std::atomic<size_t> ss_counter{0};
  const std::string tag = std::to_string(ss_counter++);
  SsPhaseTimer _pt(tag.c_str());
