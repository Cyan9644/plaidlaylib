...
// duplicate-free), then selects the k-th smallest key with the out-of-core
// randomized-pivot algorithm in Examples/external/ExternalKthSmallest.h
// (oversampled pivots -> bucket via heap_tree -> histogram_by_index -> pack the
// winning bucket -> recurse), which mirrors parlaylib's in-memory kth_smallest
// but keeps every intermediate out of core.
//
// Dual-purpose, like the benchmarks and the KMP/Rabin-Karp examples: prints
...
// so unlike KMP/Rabin-Karp there is nothing to read back element-wise -- one
// equality check suffices; keys are distinct, so the k-th smallest is unique
// and the two substrates must agree exactly).  Budget: half of physical RAM,
// override via EXAMPLE_INMEM_BUDGET_BYTES; when skipped the CSV field is left
// blank so the plotted in-mem line stops at the RAM cliff (as in KMP/Rabin-Karp
// and delayed_compare).
//
...
//
// CSV line:
// CSV,<n>,<k>,<build_s>,<select_s>,<inmem_select_s>,<result>,<throughput_gb_s>
...
//
// Complexity: O(n) work (expected), O(polylog) span per recursion level.
...
// head-to-head on identical keys.
...
int main(int argc, char* argv[]) {
  ...
  const size_t k =
      (argc > 2) ? std::stoull(argv[2]) : n / 2;  // default: median
  ...
  std::cout << "kth_smallest = " << result << "   " << std::setprecision(4)
            << select_s << "s   " << std::setprecision(2) << gb_s
            << " GB/s (input read)\n";
  ...
  if (inmem_ok) {
    ...
    std::cout
        << "in-mem parlaylib kth_smallest: skipped (~16n footprint exceeds "
  ...
  // id_/flags_/next_ recursion intermediates are cleared by run_benches's
