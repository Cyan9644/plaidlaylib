...
// ─────────────────────────────── Given a chunk_seq of `T` already sorted so
// equal-key elements are adjacent, assign each element the sorted index of its
// group head + 1 (a two-level max-scan) and emit PR{ get_pos(elem), head_index
// + 1 }.  Ranks are **1-based**: rank 0 is reserved as the beyond-end sentinel,
// which must sort strictly below every real suffix so a suffix that runs off
// the end orders before a longer one sharing its prefix.  Returns {pr,
// #distinct groups, n}; #groups == n means all keys are unique.
//
...
inline RankResult assign_ranks(const chunk_seq& sorted, const std::string& pfx,
  ...
  chunk_seq pr = ExternalTransform<T, PR>(
      sorted, pfx,
      [nc, &pre, &seed, &first_head, same, get_pos](
        ...
        for (size_t i = 0; i < n2; i++) {
          const bool flag =
              (i == 0) ? (bool)first_head[idx] : !same(in[i - 1], in[i]);
