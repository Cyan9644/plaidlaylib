...
// implementation.  The full-spectrum correctness cross-check against an
// independent oracle (parlaylib complex_fft) -- reading the entire result back
// and comparing every bin -- is an extra heavy pass (FFT's output is as large
// as its input), so it is opt-in via FFT_VERIFY=1 and exits non-zero on a
// mismatch.  Both the baseline and the compare are skipped under PLAID_TRACE
// (they must not inflate a trace) and are gated by a RAM budget of half
// physical RAM; override via EXAMPLE_INMEM_BUDGET_BYTES (0 disables both).
//
...
int main(int argc, char* argv[]) {
  ...
  //     all N bins) -- heavy (its output is as big as its input) and only
  //     useful as a differential test, so opt-in via FFT_VERIFY=1.
  // Both are skipped under PLAID_TRACE (must not inflate a trace) and both are
  // RAM-budget gated; set EXAMPLE_INMEM_BUDGET_BYTES=0 to force the baseline
  // off.
