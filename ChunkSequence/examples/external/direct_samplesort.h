    ...
    void RunIoThread() {
        ...
        while (more || in_ring > 0) {
            ...
            while (more && in_ring < kWriterRingDepth) {
                ...
                CHECK(sqe != nullptr) << "direct_sample_sort: writer ring out of sqes";
            ...
            while (in_ring > 0) {
                ...
                CHECK((size_t)cqe->res == r->bytes)
                    << "direct_sample_sort: short write (" << cqe->res << " of " << r->bytes << ")";
    ...
    std::vector<Result> ReapResult() {
        parlay::parallel_for(0, num_buckets_, [&](size_t b) {
            ...
            if (parked_bytes > 0) {
                ...
                CHECK(tail != nullptr) << "direct_sample_sort: tail alloc failed";
                size_t off = 0;
                ...
                CHECK(r->n < IO_VECTOR_SIZE) << "direct_sample_sort: no iovec left for tail";
...
private:
    ...
    Request* NewRequest(int fd, size_t offset) {
        ...
        CHECK(r != nullptr) << "direct_sample_sort: request pool closed";
        r->fd = fd;
        r->offset = offset;
        return r;
    }
