...
n+= seq.chunks[r].used; //add the used size of each chunk to the n; this tells us the number of total elements in the sequence
  ...
  static const size_t dram_budget_bytes = [] {
      ...
      if (const char* e = getenv("KTH_SMALLEST_DRAM_BUDGET_BYTES")) budget = std::stoull(e);
      return budget;
  }();

  if (n <= dram_budget_bytes){ //the whole residual (in bytes) fits DRAM -- finish here
...
//   pivots = parlay::tabulate(sample_size,[&] (long i) {return pivots[i*over];});
  ...
  std::vector<chunk_seq> buckets =
      ChunkSequenceOps::ChunkPartition<T>(seq, sample_size + 1, part_prefix, key_fn);
