...
// duplicate-free), then sorts them with the out-of-core sample sort in
...
// per bucket -> flatten the sorted buckets), which mirrors parlaylib's
// in-memory sample_sort but keeps every intermediate out of core.
//
...
// substrates must agree exactly.  Budget: half of physical RAM, override via
...
//
//   usage: external_samplesortExample [global --flags] [n]
//     n   number of keys (default 1e6)
//
// CSV line: CSV,<n>,<build_s>,<sort_s>,<inmem_sort_s>,<throughput_gb_s>
//   throughput = input bytes read / sort_s (the sort's first pass reads the
//   full n*8-byte input; deeper levels read the shrinking residual buckets).
//
// Complexity: O(n log n) work, O(polylog) span per recursion level.
