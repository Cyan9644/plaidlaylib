...
// compared element-wise against the in-memory sorted keys (as in KMP/Rabin-Karp);
// keys are distinct, so the sorted order is unique and the two substrates must
...
//   throughput = input bytes read / sort_s (the sort's first pass reads the full
//   n*8-byte input; deeper levels read the shrinking residual buckets).
//
