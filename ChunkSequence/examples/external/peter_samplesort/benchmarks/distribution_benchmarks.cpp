...
void ScatterGatherNopTest(int argc, char** argv) {
  ...
  scatter_gather.Run(
      ...
      [](size_t** ptr, size_t length) { return; }, config);
...
template <typename T>
void ScatterGatherThread(size_t num_buckets,
                         const std::function<std::pair<T*, size_t>()>& f,
  ...
  // each bucket stores a pointer to an array, which will hold temporary values
  // in that bucket
  ...
  while (true) {
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      // If we do 1, performance stays the same (40GB/s) regardless of bucket
      // size. If we do 2, interesting things happen. BW goes from 80GB/s (1, 2,
      // 4, ..., 256 buckets) to 40GB/s (512, 1024, 2048) to 14GB/s (4096).
...
void ScatterGatherNoIOTest(int argc, char** argv) {
  ...
  parlay::parallel_for(
      ...
      [&](size_t i) { ScatterGatherThread<T>(num_buckets, generator, reader); },
      1);
  double time = timer.next_time();
