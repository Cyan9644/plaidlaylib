...
void OrderedFileWriterTest(int argc, char** argv) {
  ...
  parlay::par_do(
      [&]() {
        parlay::parallel_for(
            0, 2, [&](size_t i) { writer.RunIOThread(&writer); }, 1);
      },
      [&]() {
        ...
        parlay::parallel_for(
            0, TOTAL_WRITE_SIZE / SINGLE_WRITE_SIZE,
            [&](size_t i) {
...
void UnorderedReadTest(int argc, char** argv) {
  ...
  reader.Start(
      UnorderedReaderConfig(num_io_threads, io_uring_size * 2, io_uring_size));
  ...
  double throughput =
      (double)(expected_size - remaining_size) / 1e9 / time_elapsed;
...
void UnorderedWriteTest(int argc, char** argv) {
  CHECK(argc > 3) << "Usage: " << argv[0] << " " << argv[1]
                  << " <file prefix> <size (pow of 2)>";
  using Type = long long;
...
void RandomReadTest(int argc, char** argv) {
  ...
  auto queries = parlay::tabulate(n, [&](size_t i) -> size_t {
    auto g = gen[i];
