...
/**
 * Not really an ordered file writer. This class creates many buckets. Each
 * bucket corresponds to a file. Data sent to the same file are not written in
 * order, but the order of the buckets are respected.
 *
 * @tparam T
 */
...
class OrderedFileWriter {
 ...
 public:
  ...
  OrderedFileWriter(const std::string& prefix, size_t num_buckets,
  ...
  // For debugging: setting it to true will skip the write to disk step and
  // deallocate right before
  bool skip_write = false;

  static void RunIOThread(
      OrderedFileWriter<T, SAMPLE_SORT_BUCKET_SIZE>* writer) {
    ...
    while (has_more_requests || requests_in_ring > 0) {
      bool reap_required = requests_in_ring >= RING_DEPTH ||
                           !has_more_requests || completions->IsEmptyUnsafe();
      ...
      while (requests_in_ring < RING_DEPTH) {
        ...
        if (sqe == nullptr) {
          [[unlikely]] LOG(ERROR) << "io_uring does not have enough sqe";
          return;
        }
        io_uring_sqe_set_data(sqe, request);
        io_uring_prep_writev(sqe, request->fd, &request->io_vectors[0],
                             request->iovec_count, request->offset);
        requests_in_ring++;
        need_submit = true;
      }
      if (need_submit) {
  ...
  inline void SubmitRequest(IOVectorRequest* r) { pending_requests.Push(r); }

  /**
   * Initialize the file writer
   *
   * @param prefix Prefix of output file names
   * @param bucket_count Number of buckets to create (they're numbered starting
   * from 0)
   * @param file_flush_threshold The threshold (in bytes) at which a chunk of
  ...
  void Initialize(const std::string& prefix, size_t bucket_count,
    ...
    requests =
        (IOVectorRequest*)malloc(request_pool_size * sizeof(IOVectorRequest));
    ...
    for (size_t i = 0; i < bucket_count; i++) {
      ...
      // do a placement new since the std::mutex in a BucketData can't be copied
      // or moved
  ...
  /**
   * Clean up half-full buckets and misaligned writes in each bucket. Must be
   * called before writer exits to ensure that everything is written to disk.
  ...
  void CleanUp() {
    ...
    parlay::parallel_for(
        0, num_buckets,
        [&](size_t i) {
          auto [buffer, buffer_size, true_size] =
              buckets[i].GatherMisalignedPointers();
          ...
          result_files[i].true_size =
              bucket->file_size - buffer_size + true_size;
        },
        1);
   ...
   *
   * Do not submit arrays whose size is not a multiple of O_DIRECT_MULTIPLE
   * unless necessary.
   * @param bucket_number
   * @param pointer Ownership of pointer will now be assumed by the writer,
   * which will deallocate it using Free.
   * @param count
   */
  void Write(size_t bucket_number, T* pointer, size_t count) {
    CHECK(bucket_number < num_buckets) << "Invalid bucket number";
    CHECK((size_t)pointer % O_DIRECT_MULTIPLE == 0)
        << "Write buffers must be aligned";
    ...
    if (request->current_size >= io_threshold ||
        request->iovec_count >= IO_VECTOR_SIZE) {
      bucket->file_size += request->current_size;
 ...
 private:
  ...
  struct IOVectorRequest {
    ...
    void Reset() {
      for (size_t i = 0; i < iovec_count - (last_request ? 1 : 0); i++) {
        BucketAllocator::free(
            reinterpret_cast<BucketData*>(io_vectors[i].iov_base));
      }
      if (last_request) {
  ...
  struct Bucket {
    ...
    explicit Bucket(std::string& file_name) {
      current_file =
          open(file_name.c_str(), O_WRONLY | O_DIRECT | O_CREAT, 0644);
      SYSCALL(current_file);
    }

    ~Bucket() { SYSCALL(close(current_file)); }

    /**
     * Write misaligned pointers (those whose size cannot be divided by
     * O_DIRECT_MULTIPLE) to the file.
     *
     * Copies the content in each pointer into a buffer and then writes the
     * buffer to disk using synchronous IO.
     */
    std::tuple<unsigned char*, size_t, size_t> GatherMisalignedPointers() {
      ...
      size_t target_write_size =
          (write_size / O_DIRECT_MULTIPLE + 1) * O_DIRECT_MULTIPLE;
      if (target_write_size - write_size < METADATA_SIZE) {
        target_write_size += O_DIRECT_MULTIPLE;
      }
      auto* write_buffer = (unsigned char*)std::aligned_alloc(
          O_DIRECT_MEMORY_ALIGNMENT, target_write_size);
      ...
      // compute true file size (excluding garbage bytes and marker at the end)
      // and file size on disk
