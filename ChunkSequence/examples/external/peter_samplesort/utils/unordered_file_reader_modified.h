...
//   * PrepFiles accepts a vector of chunk headers (duck-typed: each element
//   must
...
//         (data_ptr, valid_element_count, file_index, index, which_chunk,
//         filename)
...
struct UnorderedChunkReaderConfig {
  ...
  UnorderedChunkReaderConfig(size_t num_threads, size_t max_requests,
                             size_t queue_depth)
 ...
 * chunk_header (filename, begin_address, used, index) and is read
 * independently.
 *
...
class UnorderedChunkReader {
 public:
  ...
  typedef std::tuple<T*, size_t, size_t, size_t, size_t, std::string>
      BufferData;
  ...
  void PrepFiles(const Headers& headers) {
    ...
    for (const auto& h : headers) {
      chunks.push_back(
          ChunkRequest{h.filename, h.begin_address, h.used, h.index});
    }
  }

  void Start(
      const UnorderedChunkReaderConfig& config = UnorderedChunkReaderConfig()) {
    ...
    for (size_t i = 0; i < config.num_threads; i++) {
      ...
      worker_threads.push_back(std::make_unique<std::thread>(
          RunFileReaderWorker, this, std::move(chunk_list), config.queue_depth,
          config.max_requests));
    }
  }
  ...
  void Push(T* data, size_t size, size_t file_index, size_t index,
    ...
    CHECK((size_t)data % O_DIRECT_MULTIPLE == 0)
        << "Buffers used by the UnorderedChunkReader must be aligned.";
    buffer_queue.Push(
        {data, size, file_index, index, which_chunk, std::move(filename)});
  }
 ...
 private:
  ...
  static void RunFileReaderWorker(UnorderedChunkReader* reader,
    ...
    while (completed < my_chunks.size() && reader->is_open) {
      ...
      while (outstanding_requests > 0) {
        ...
        if (wait_result == 0) {
          ...
          reader->Push(request->data, request->valid_elems, request->chunk_idx,
                       c.index, c.begin_address / READ_SIZE, c.filename);
      ...
      while (!available_requests.empty() && next_chunk < my_chunks.size()) {
        ...
        io_uring_prep_read(sqe, request->fd, request->data, read_size,
                           c.begin_address);
    ...
    CHECK(completed == my_chunks.size())
        ...
        << my_chunks.size() << " chunks total, yet " << completed
