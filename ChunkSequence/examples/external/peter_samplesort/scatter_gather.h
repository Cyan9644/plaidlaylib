...
class ScatterGather {
 ...
 private:
  ...
  /**
   * Keep polling from the reader for data and then assign items into buckets
   * according to how they compare with samples. Buckets are flushed to the
   * writer whenever they're full.
   *
   * The function is meant to be run as a thread and exits when the reader
   * returns nullptr (no more input available)
   *
   * @tparam Comparator
   * @param samples
   * @param flush_threshold The number of bytes in each bucket. This is the
   * threshold at which a bucket gets sent to the writer. It may or may not be
   * written to disk at the writer's discretion.
   * @param comp
   */
  void AssignToBucket(size_t num_buckets, const AssignerFunction assigner,
                      const std::vector<FileInfo>& files) {
    // reads from the reader and put result into a thread-local buffer; send to
    // intermediate_writer when buffer is full
    size_t buffer_size = SAMPLE_SORT_BUCKET_SIZE / sizeof(T);
    // each bucket stores a pointer to an array, which will hold temporary
    ...
    while (true) {
      ...
      for (size_t i = 0; i < size; i++) {
        ...
        if (buffer_index[bucket_index] == buffer_size) {
          ...
          intermediate_writer.Write(bucket_index, buckets[bucket_index],
                                    buffer_size);
   ...
   * @tparam Comparator
   * @param file_info
   * @param target_file File to which the result of the sorting algorithm will
   * be written
   * @param comparator
   * @return Information of the resulting file
   */
  FileInfo ProcessBucket(const FileInfo& file_info,
  ...
  parlay::sequence<FileInfo> QueuePhase2(
      const std::string& result_prefix, const ProcessorFunction& processor,
  ...
  parlay::sequence<FileInfo> WorkerOnlyPhase2(
      const std::string& result_prefix, const ProcessorFunction& processor,
      const std::vector<FileInfo>& bucket_list) {
    struct LocalFile {
      int fd;
    ...
    parlay::parallel_for(
        ...
        [&](size_t worker_id) {
          ...
          bool need_reap_read = false, need_submit_read = true,
               need_process = false, need_reap_write = false,
          ...
          while (need_submit_read || need_reap_write) {
            ...
            if (need_submit_read) {
              ...
              io_uring_prep_read(sqe, next.fd, next.buffer, file_info.file_size,
                                 0);
            ...
            if (need_process) {
              ...
              current.info.file_name =
                  GetFileName(result_prefix, current.info.file_index);
              int fd = open(current.info.file_name.c_str(),
                            O_DIRECT | O_WRONLY | O_CREAT, 0644);
              SYSCALL(fd);
              current.fd = fd;
  ...
  parlay::sequence<FileInfo> SimplePhase2(
      const std::string& result_prefix, const ProcessorFunction& processor,
      const std::vector<FileInfo>& bucket_list) {
 ...
 public:
  std::vector<FileInfo> Run(std::vector<FileInfo>& input_files,
    ...
    parlay::par_do(
        ...
        [&]() {
          parlay::parallel_for(
              0, parlay::num_workers() - intermedia_io_threads,
              [&](int i) {
    ...
    // Print detailed statistics under benchmark mode, otherwise just print the
    // time
    ...
    parlay::sequence<FileInfo> results =
        WorkerOnlyPhase2(result_prefix, processor, bucket_list);
    if (config.benchmark_mode) {
      double throughput = GetThroughput(input_files, timer.next_time());
