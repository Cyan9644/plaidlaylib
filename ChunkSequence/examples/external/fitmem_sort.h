...
// DRAM, so it materializes and sorts each bucket directly rather than
// recursing.
...
chunk_seq fitmem_sort(chunk_seq& seq, Less less1 = {}) {
  ...
  // Route every key into its bucket in a single streaming pass over seq,
  // deriving the bucket from the value (ss.rank) on the fly.  The earlier
  // version first materialized a full size_t bucket-id chunk_seq to disk with
  // ChunkMap (an 8n write + 8n read) purely to feed count_sort2; folding the
  ...
  // Each bucket is assumed to fit in DRAM: pull it in, sort it in place, and
