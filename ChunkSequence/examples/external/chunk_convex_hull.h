...
// "shrink the working set until it fits, then go in-memory" pattern the
// external sort uses; near the root the input is huge and out-of-core, near the
// leaves it is small and in-DRAM.
//
...
// engine and DensePack are generic in the element size (only the *delayed*
// layer is limited to <=8 B), so nothing here is special-cased for the fat
// element. `idx` carries the point's original position so the result is a list
// of hull vertex indices, matching upstream `upper_hull`'s output (and its
// tie-break: among equally-far points the one with the larger original index
// wins, which is exactly upstream's `maximum<pair<double,int>>` on (area,
// index)).
//
...
// path string that would grow with recursion depth and blow the filename
// limit).
...
// In-DRAM upper-hull quickhull on the points strictly above the line l--r (in
// any order).  A faithful port of upstream quickhull.h that carries the points
// themselves (so no back-reference into a global array) and returns original
// indices.  The farthest point is chosen by (area, idx) (FarPointMonoid), so
// the result is identical regardless of the order S arrives in.
//
...
// freeing the parent the parallel recursion would hold O(n log n) live points
// at once and blow up DRAM on large base cases.  With it, peak stays O(n).
inline parlay::sequence<uint64_t> qh_mem(parlay::sequence<hpoint> S, hpoint l,
                                         hpoint r) {
  const long n = (long)S.size();
  if (n == 0) return parlay::sequence<uint64_t>();
  if (n == 1) return parlay::sequence<uint64_t>(1, S[0].idx);

  // Farthest point from l--r; delayed keys, so the pairs are never
  // materialized.
  const hpoint mid =
      parlay::reduce(
          parlay::delayed_tabulate(
              n, [&](long i) { return FarPoint{area(l, r, S[i]), S[i]}; }),
          FarPointMonoid{})
          .pt;

  auto left =
      parlay::filter(S, [&](const hpoint& p) { return area(l, mid, p) > 0; });
  auto right =
      parlay::filter(S, [&](const hpoint& p) { return area(mid, r, p) > 0; });
  S = parlay::sequence<hpoint>();  // free the parent's points before recursing

  parlay::sequence<uint64_t> LR, RR;
  parlay::par_do_if(
      n > 1000, [&] { LR = qh_mem(std::move(left), l, mid); },
...
// running best (largest (area(l,r,p), p.idx)), and combine two per-worker
// bests.
struct FarMonoid {
  hpoint l, r;
  struct Acc {
...
// Combined min/max-by-(x,y) as a ChunkReduce monoid (upstream's
// `minmax_element` with the same (x, then y) comparator).  Endpoints of the
// top-level hull.
struct MinMaxMonoid {
  struct Acc {
    hpoint mn, mx;
...
// Recursive out-of-core quickhull over the points strictly above l--r (`pts`,
// any order).  Returns the hull vertices strictly between l and r, left to
// right, as original indices.  Frees its own scratch.
inline std::vector<uint64_t> quickhull_ext(const chunk_seq& pts, hpoint l,
  ...
  // pass that routes each point to the left sub-region (above l--mid), the
  // right sub-region (above mid--r), or drops it (inside triangle l,mid,r). The
  // two sub-regions are disjoint (mid is the apex, so no point above l--r is
  // above both new lines), so a single ChunkPartition replaces the two
  // ChunkFilters: one read pass and one reader/writer instead of two of each.
  ...
  std::vector<chunk_seq> parts = ChunkPartition<hpoint>(
      pts, /*num_buckets=*/2, sp, [l, mid, r](const hpoint& p) -> size_t {
 ...
 *                          out-of-core reduce+partition primitives.  Clamped so
 * at least one element is always a valid base case.
 * @param scratch_prefix   file-name prefix for the recursion's intermediate
 *                          chunk_seqs (all cleaned up before returning).
 */
inline std::vector<uint64_t> UpperHull(
  ...
  // Points strictly above the min--max line: the upper-hull interior
  // candidates. A 1-bucket ChunkPartition (keep-or-drop) so this too is a
  // single-reader pass
  ...
  chunk_seq above = ChunkPartition<hpoint>(
      points, /*num_buckets=*/1, ap, [minp, maxp](const hpoint& p) -> size_t {
