...
// parlay::hash64), then computes the suffix array — the n suffix start positions
// in lexicographic order — with ChunkSuffixArray (examples/external/
// chunk_suffix_array.h): a sort-based prefix doubling whose every random
// rank[p+offset] gather is realized as a streaming sort-join, so nothing but the
// per-chunk boundary metadata is ever resident.  The heavy lifting is
...
// the benchmarks.  Budget: half of physical RAM (the in-mem build's working set
// is a small multiple of n), override via EXAMPLE_INMEM_BUDGET_BYTES; the
// baseline is additionally capped at n < 2^31 (upstream indexes with unsigned
// int).  When skipped the CSV field is left blank so the plotted in-mem line
// stops at the RAM cliff.
//
