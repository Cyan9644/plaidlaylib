...
// lying exactly on a known line (deterministic from a per-index
// parlay::random_generator, so it is reproducible without any shared mutable
// state), then fits a line to them with the out-of-core algorithm in
// examples/external/external_linefit.h (zip x and y into one delayed sequence
// -> reduce for the centroid -> map to squared/cross deviations -> reduce again
// for the slope), which mirrors parlaylib's in-memory linefit
// (examples/in_memory/linefit.h) but never materializes the zipped points or
// the deviation map on disk -- both passes are fully delayed, so fitting reads
// x and y exactly once each and writes nothing.
//
...
// baseline.  Unlike the integer examples (kth_smallest, sample_sort), the
// result here is a pair of doubles produced by two summations whose term order
// differs between the chunked out-of-core reduce and parlaylib's in-memory
// reduce, so a bit-exact cross-check would be wrong; the two are compared
// within a relative tolerance instead.  Budget: half of physical RAM, override
// via EXAMPLE_INMEM_BUDGET_BYTES; when skipped the CSV field is left blank so
// the plotted in-mem line stops at the RAM cliff (as in the other examples).
//
...
int main(int argc, char* argv[]) {
  ...
  if (inmem_ok) {
    ...
    std::cout << "in-mem parlaylib linefit: offset = " << offset_mem
              << "   slope = " << slope_mem << "   " << std::setprecision(4)
              << inmem_fit_s << "s\n";
    if (!close(offset, offset_mem) || !close(slope, slope_mem)) {
      ...
      std::cout << "cross-check: out-of-core fit matches in-mem fit (within "
                   "tolerance)\n";
    }
  } else {
