...
//
// This is external_samplesort_vs_peter.cpp with our contestant swapped: that
// one times the sort built on the library's primitives, this one times the sort
// written directly against the I/O layer in the same shape as Peter's.  Run
...
// with the chunk_seq code.  peter_shim.{h,cpp} isolate Peter's sort behind a
...
// (15-25% on the dev box).  VS_PETER_FIRST rotates which sort goes first; it is
// a knob for *checking* that the teardown works — the two times must not move
// when you flip it — not a measurement knob.
//
//   usage: direct_samplesort_vs_peterExample [global --flags] [n]
//     n               number of keys (default 1e6; must be a multiple of 512)
//     VS_PETER_FIRST  which sort goes first, 0 or 1 (default 0 = Peter first):
//     a
...
// clear_drives() removes a sort's files and then settles.  The wait is what
// lets the second sort here be timed on the same terms as the first: unlink()
// returns long before ext4 has freed the blocks, and a sort started on top of
// that background work runs 15-25% slow (dev box) — enough to flip this
// comparison.
...
// clear_drives() sweeps by name prefix rather than enumerating
// GetFileName(prefix, d), because the sorts leave tag-suffixed intermediates a
// fixed 0..num_drives enumeration would miss.  This is how we guarantee each
// timed sort runs on drives cleared of the *other* substrate's files (and of
// any stale run).

// The two substrates' on-disk file families (see run_benches data_globs). Ours:
// the dss_in input + the sort's per-bucket output files (dss<tag>_<b>), which
// ARE the returned sequence, so this prefix must be swept or the whole output
// leaks on the drives every sweep point.  Peter's: pss_in/pss_out + his
...
// back, and sweep every file it put on the drives.  Two of these race here
// (Peter's and ours), run in the order VS_PETER_FIRST picks; the results are
// stored per contestant (not per run position), so the CSV columns stay fixed
// either way.
struct Sorter {
  std::string name;
...
int main(int argc, char* argv[]) {
  ...
  // Held across the loop: Peter's per-drive output file list (his Sort fills
  // it, his ReadBackSorted consumes it) and our chunk_seqs (materialize reads
  // the sorted output back before its files are cleared).
  ...
  // — handing the next one drives clear of the previous sort's data.  Under the
  // RAM budget each output is read into DRAM before its files are cleared, so
  // the element-wise cross-check still sees both.
  std::cout << std::fixed;
  ...
  if (check_ok) {
    ...
    if (agree)
      std::cout << "cross-check: both out-of-core outputs match the sorted "
                   "reference\n";
  } else {
  ...
  // Columns:
  // n,ext_build_s,ext_sort_s,peter_build_s,peter_sort_s,ext_gb_s,peter_gb_s
  auto f9 = [](double v) {
    std::ostringstream o;
  ...
  std::cout << "CSV," << n << ',' << f9(sorters[1].build_s) << ','
            << f9(sorters[1].sort_s) << ',' << f9(sorters[0].build_s) << ','
            << f9(sorters[0].sort_s) << ',' << f9(gb / sorters[1].sort_s) << ','
            << f9(gb / sorters[0].sort_s) << '\n';
