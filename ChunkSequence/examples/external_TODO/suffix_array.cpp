// Example: out-of-core suffix array construction via ChunkSuffixArray.
//
// Builds an n-char text across the SSDs (4-letter alphabet, deterministic from
// parlay::hash64), then computes the suffix array — the n suffix start
// positions in lexicographic order — with ChunkSuffixArray (examples/external/
// chunk_suffix_array.h): a sort-based prefix doubling whose every random
// rank[p+offset] gather is realized as a streaming sort-join, so nothing but
// the per-chunk boundary metadata is ever resident.  The heavy lifting is
// direct_sample_sort (examples/external/direct_samplesort.h).
//
...
// examples sweep (make bench-examples) times the construction across a sweep of
// n.
//
...
// DRAM baseline, and cross-checks the full array element-wise (read back off
// the drives; exits non-zero on a mismatch) — the differential test in the
// spirit of the benchmarks.  Budget: half of physical RAM (the in-mem build's
// working set is a small multiple of n), override via
// EXAMPLE_INMEM_BUDGET_BYTES; the baseline is additionally capped at n < 2^31
// (upstream indexes with unsigned int).  When skipped the CSV field is left
// blank so the plotted in-mem line stops at the RAM cliff.
//
//   usage: suffix_arrayExample [global --flags] [n]
...
#include "ChunkSequence/examples/external_TODO/chunk_suffix_array.h"
...
// Deterministic 4-letter text: char i, computable anywhere (non-zero chars, as
// upstream requires).
...
// compare every value.
static bool contents_equal(const chunk_seq& seq,
                           const parlay::sequence<unsigned int>& expected) {
  ...
  CHECK(buf != nullptr) << "suffix_array: readback buffer alloc failed";
  size_t j = 0;
  bool ok = true;
...
int main(int argc, char* argv[]) {
  ...
  CHECK(n > 0 && n < (size_t{1} << 32)) << "suffix_array: need 0 < n < 2^32";

  // RAM budget for the in-memory parlaylib baseline: its working set is a small
  // multiple of n (a 16n pair-sort during the initial pack, plus ~9n of
  // rank/flag/sorted arrays), and the differential check reads back both the
  // out-of-core (4n) and reference (4n) arrays.
  const size_t phys =
  ...
  const std::string text_prefix = "sa_text";
  const std::string out_prefix = "sa_out";
  ...
  std::cout << "|SA| = " << count << "   " << std::setprecision(4) << sa_s
            << "s   " << std::setprecision(2) << gb_s
            << " GB/s (text size / build time)\n";
  ...
  if (inmem_ok) {
    auto text_mem =
        parlay::tabulate(n, [](size_t i) { return (unsigned char)text_at(i); });
    t0 = Clock::now();
    ...
    if (sa_mem.size() != count) {
      ...
      std::cout << "*** MISMATCH: in-mem suffix array differs from out-of-core "
                   "output ***\n";
      agree = false;
    } else {
    ...
    std::cout << "in-mem parlaylib suffix_array: skipped (n exceeds RAM/index "
                 "budget)\n";
  }
