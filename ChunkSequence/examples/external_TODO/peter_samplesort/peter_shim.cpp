...
double BuildInput(const std::string& prefix, std::size_t n) {
  ...
  CHECK(n % 512 == 0) << "peter_shim::BuildInput needs n % 512 == 0 for "
                         "O_DIRECT alignment; got "
                      << n;
  ...
  const std::size_t num_files =
      std::max<std::size_t>(1, std::min(num_drives, total_blocks));
  ...
  for (std::size_t f = 0; f < num_files; f++) {
    const std::size_t blocks_f =
        total_blocks / num_files + (f < total_blocks % num_files ? 1 : 0);
  ...
  parlay::parallel_for(
      ...
      [&](std::size_t f) {
        ...
        constexpr std::size_t kBufElems =
            (4u << 20) / sizeof(uint64_t);  // 4 MiB
...
double Sort(const std::string& in_prefix, const std::string& out_prefix,
  ...
  CHECK(!input.empty()) << "peter_shim::Sort found no input files for prefix "
                        << in_prefix;
