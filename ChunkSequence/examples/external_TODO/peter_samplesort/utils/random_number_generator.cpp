...
template <typename T>
parlay::sequence<T> GenerateExponentialDistribution(size_t count,
                                                    double lambda) {
  parlay::random_generator random;
...
template parlay::sequence<uint64_t> GenerateExponentialDistribution(size_t,
                                                                    double);
template parlay::sequence<int64_t> GenerateExponentialDistribution(size_t,
                                                                   double);
template parlay::sequence<uint32_t> GenerateExponentialDistribution(size_t,
                                                                    double);
template parlay::sequence<int32_t> GenerateExponentialDistribution(size_t,
                                                                   double);
...
template <typename T>
void WriteNumbers(const std::string& prefix, size_t n,
                  const std::function<parlay::sequence<T>(size_t)>& generator) {
  UnorderedFileWriter<T> writer(prefix);
  constexpr auto WRITE_SIZE = 4 << 20;
  const auto step = WRITE_SIZE / sizeof(T);
  parlay::parallel_for(
      ...
      [&](size_t i) {
        T* buffer =
            (T*)std::aligned_alloc(O_DIRECT_MEMORY_ALIGNMENT, WRITE_SIZE);
...
template <typename T>
void GenerateZipfianRandomNumbers(const std::string& prefix, size_t n,
                                  double s) {
...
template <typename T>
void GenerateExponentialRandomNumbers(const std::string& prefix, size_t n,
                                      double s) {
  WriteNumbers<T>(prefix, n, [&](size_t sz) {
...
void GenerateUniformRandomNumbers(const std::string& prefix, size_t count,
  ...
  UnorderedFileWriter<T> writer(prefix);
  CHECK(count % GRANULARITY == 0)
      << "Can only generate numbers that are a multiple of " << GRANULARITY;
  ...
  parlay::parallel_for(
      ...
      [&](size_t _) {
        ...
        while (true) {
          ...
          std::shared_ptr<T> result((T*)aligned_alloc(O_DIRECT_MEMORY_ALIGNMENT,
...
template void GenerateUniformRandomNumbers<int64_t>(const std::string& prefix,
                                                    size_t count,
                                                    int64_t limit);

template void GenerateUniformRandomNumbers<uint64_t>(const std::string& prefix,
                                                     size_t count,
                                                     uint64_t limit);

template void GenerateUniformRandomNumbers<int32_t>(const std::string& prefix,
                                                    size_t count,
                                                    int32_t limit);

template void GenerateUniformRandomNumbers<uint32_t>(const std::string& prefix,
                                                     size_t count,
                                                     uint32_t limit);

template void GenerateZipfianRandomNumbers<size_t>(const std::string& prefix,
                                                   size_t n, double s);

template void GenerateExponentialRandomNumbers<size_t>(
    const std::string& prefix, size_t n, double s);
