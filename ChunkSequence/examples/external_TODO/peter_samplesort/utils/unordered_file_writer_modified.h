...
// Modified variant of UnorderedChunkWriter used by Plaidlay/externalFilter.h.
//
// Difference from the original utils/unordered_file_writer.h: per-file byte
// counters are atomic. externalFilter runs the writer with multiple worker
// threads that can all be handed requests targeting the same file_index, so the
// shared bytes_issued/bytes_written counters would otherwise be a data race.
// externalFilter always supplies an explicit file_offset, so the auto-append
// path is not used by it, but the counters are still touched concurrently.

#ifndef SORTING_UNORDERED_FILE_WRITER_MODIFIED_H
#define SORTING_UNORDERED_FILE_WRITER_MODIFIED_H
...
struct UnorderedChunkWriterConfig {
  ...
  UnorderedChunkWriterConfig() = default;

  UnorderedChunkWriterConfig(size_t io_uring_size, size_t queue_size,
...
template <typename T>
class UnorderedChunkWriter {
 private:
  struct WriteRequest;
  struct OpenedFile;

 public:
  UnorderedChunkWriter() = default;

  explicit UnorderedChunkWriter(
      const std::string& prefix,
      const UnorderedChunkWriterConfig& config = UnorderedChunkWriterConfig()) {
    Start(prefix, config);
  }
  ...
  void Start(const std::vector<std::string>& file_names,
             const UnorderedChunkWriterConfig& config) {
  ...
  void Push(std::shared_ptr<T> data, size_t size, size_t file_index = -1,
    ...
    CHECK((size_t)data.get() % O_DIRECT_MULTIPLE == 0)
        << "Buffers used by the UnorderedChunkWriter must be aligned.";
    auto request =
 ...
 private:
  ...
  struct OpenedFile {
    int fd;
    std::atomic<size_t> bytes_issued{0};
  ...
  static void RunFileWriterWorker(UnorderedChunkWriter* writer,
    ...
    while (phase != ALL_DONE) {
      ...
      while (outstanding_request > 0) {
        ...
        if (wait_result == 0) {
          ...
          file->bytes_written.fetch_add(request->size * sizeof(T));
      ...
      while (outstanding_request < max_outstanding_requests &&
        ...
        // Explicit offset (the externalFilter path): write exactly where
        // the caller asked and leave the append cursor untouched.
        // Otherwise atomically reserve the next append slot.
        size_t offset;
        if (request->file_offset != (size_t)-1) {
          offset = request->file_offset;
        } else {
          offset = file->bytes_issued.fetch_add(num_bytes);
        }
