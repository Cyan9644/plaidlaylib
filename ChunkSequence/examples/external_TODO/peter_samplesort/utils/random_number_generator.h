...
template <typename T>
parlay::sequence<T> GenerateZipfianDistribution(
    size_t n, double s, T limit = std::numeric_limits<T>::max());

template <typename T>
void GenerateZipfianRandomNumbers(const std::string& prefix, size_t count,
                                  double s);
...
template <typename T>
void GenerateExponentialRandomNumbers(const std::string& prefix, size_t count,
                                      double s);
