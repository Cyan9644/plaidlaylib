...
parlay::sequence<T> RandomBatchRead(const std::vector<FileInfo>& files,
  ...
  for (size_t i = 0; i < num_files; i++) {
    ...
    if (files[i].true_size == 0) {
      [[unlikely]];
      LOG(ERROR)
          << "File's true size is not determined or an empty file is passed: "
          << files[i].file_name;
    }
  }
  ...
  // open all files in parallel; could be sped up using async open calls, but
  // performance gain is probably negligible
  int fds[num_files];
  parlay::parallel_for(
      0, num_files,
      [&](size_t i) {
  ...
  return parlay::flatten(parlay::map(
      ...
      [&](size_t segment) {
        ...
        // If there are too many threads, io_uring_queue_init will fail due to
        // insufficient locked memory
        const unsigned IO_URING_ENTRIES = 512;
        struct io_uring ring;
        SYSCALL(io_uring_queue_init(IO_URING_ENTRIES, &ring,
        ...
        while (i < segment_end || requests_in_ring > 0) {
          ...
          while (i < segment_end && !free_buffers.empty()) {
            ...
            // Start and end of aligned read (both multiples of
            // O_DIRECT_MULTIPLE)
            size_t start = AlignDown(byte_offset),
                   end = AlignUp(byte_offset + sizeof(T));
            size_t buffer_index = free_buffers.back();
            free_buffers.pop_back();
            buffers[buffer_index].offset = byte_offset - start;
            auto file_num =
                std::upper_bound(size_prefix_sum, size_prefix_sum + num_files,
                                 start) -
                size_prefix_sum;
            CHECK((size_t)file_num < num_files);
            struct io_uring_sqe* sqe = io_uring_get_sqe(&ring);
            io_uring_prep_read(
                sqe, fds[file_num], buffers[buffer_index].buffer, end - start,
          ...
          // at this point, we have submitted as much as we can; now is time to
          // reap
          bool wait = true;
          while (requests_in_ring > 0) {
            ...
            results.push_back(
                *reinterpret_cast<T*>(buffer->buffer + buffer->offset));
          }
        }
