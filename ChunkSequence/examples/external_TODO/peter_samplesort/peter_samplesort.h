...
class SampleSort {
 private:
   ...
   */
  static std::vector<T> GetPivots(const std::vector<FileInfo>& file_list,
                                  size_t num_pivots) {
    ...
    if (num_pivots > n) {
      [[unlikely]] LOG(ERROR) << "Sample size is " << num_pivots
                              << " but we only have " << n << " elements";
    }
    // too many samples means the buckets are distributed too evenly; this will
    // affect performance later on
    size_t oversample_size =
        std::min(n, num_pivots * (size_t)std::sqrt(num_pivots));
    parlay::random_generator generator;
    std::uniform_int_distribution<size_t> dis(0, n - 1);
    auto samples = parlay::sort(RandomBatchRead<T>(
        file_list, parlay::map(parlay::iota(oversample_size), [&](size_t i) {
    ...
    while (remaining_pivots > 0) {
      i += std::max(1UL, (oversample_size - i - remaining_pivots) /
                             (remaining_pivots + 1));
  ...
  /**
   * Compute a sensible sample size based on information about the files that
   * are about to be sorted.
  ...
  static size_t GetSampleSize(const std::vector<FileInfo>& input_files) {
    ...
    //   (1) samples should ideally fit in L1 cache for maximal binary search
    //   efficiency (2) each bucket should be small enough to fit in main
    //   memory; ideally they should be small each that we
    ...
    // max sample size cannot exceed the number of elements; it should also not
  ...
  static inline size_t BinarySearch(const parlay::sequence<T>& pivots,
                                    const T& t, Comparator comp) {
    return (size_t)std::distance(
        pivots.begin(),
        std::upper_bound(pivots.begin(), pivots.end(), t, comp));
  }
  ...
  struct SimpleAssigner {
    ...
    explicit SimpleAssigner(const parlay::sequence<T>& pivots,
                            const Comparator comp)
  ...
  struct DeduplicatingAssigner {
    ...
    explicit DeduplicatingAssigner(const parlay::sequence<T>& pivots,
                                   const Comparator comp)
        : pivots(pivots), comp(comp) {
      T prev = pivots[0];
      uint32_t count = 1;
 ...
 public:
  ...
  std::vector<FileInfo> Sort(std::vector<FileInfo>& input_files,
    ...
    auto results =
        scatter_gather.Run(input_files, result_prefix, assigner.GetAssigner(),
                           simple_processor, config);
