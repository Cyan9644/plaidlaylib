...
chunk_seq sample_sort(chunk_seq& seq, Less less1 = {}) {
  ...
  auto ids = ChunkMap<T, size_t>(
      seq, "ss_id_" + tag,
      [&](T e) {  // problem: the dual reader can only deal with
        // a single type, so we're going to have to waste by using size_t
        // instead of
  ...
  parlay::parallel_for(0, num_buckets, [&](long i) {
    ...
    // recurring on this samplesort chain is pretty expensive due to the buffer
    // allocation we might consider just an external quicksort for this level,
    // as it would use much less memory. probably multiple levels of parallelism
    // won't help much regardless since we need to do reads for the pivots =
    // large overhead on recurring calls

    // so instead we're going to call the external quicksort method, which we
    // actually have a method for now but it's implemented using primitives. to
    // get the best performance out of this example, we're going to want to
    // implement it manually with a reader/writer
    externalSequenceVector[i] =
