...
//     gathers are realized by scattering (target_pos, srank) records and
//     pairing them against a per-position base stream (a slot-keyed sort, the
//     same trick the prefix-doubling example uses for rank[p+offset]).
//
...
//   1. triples : for every sample R-index r emit (s[i], s[i+1], s[i+2]) with
//   the
...
//
// A DRAM base case (shrink-until-it-fits, like chunk_convex_hull.h) finishes
// any level whose text fits a byte budget with an in-memory Kärkkäinen–Sanders
...
#include "ChunkSequence/examples/external_TODO/chunk_sa_common.h"
...
static_assert(CHUNK_SIZE % sizeof(Triple) == 0,
              "Triple must divide CHUNK_SIZE");
...
static_assert(sizeof(ByR) == sizeof(sa_detail::PR) &&
                  sizeof(ByName) == sizeof(sa_detail::PR),
              "ByR/ByName reinterpret PR");

// Position-keyed join record.  slot 0 = BASE (one per position, carries chars),
// slot 1 = VAL (one per sample, carries its srank).  (target,slot) is unique,
// so the sort has no duplicate keys.  p0/p1 are a payload union: BASE=(c0,c1),
// VAL=(srank,0).
struct Join {
  ...
  bool operator==(const Join& o) const {
    return target == o.target && slot == o.slot;
  }
};
...
struct SufRec {
  ...
  bool operator<(const SufRec& b) const {
    ...
    if (ra != 0 && rb != 0)
      return self < b.self;    // both sample: recursion rank
...
static_assert(CHUNK_SIZE % sizeof(SufRec) == 0,
              "SufRec must divide CHUNK_SIZE");

// ── DC3 index arithmetic (mirrors the Kärkkäinen–Sanders reference)
// ─────────── Sample R-indices r in [0, n02): the left half [0, n0) are
// pos%3==1 suffixes, the right half [n0, n02) are pos%3==2.  A dummy left-half
// entry at r in [n1, n0) (present iff n%3==1) maps to the pad position n.
...
// ── forward halo: first two elements of every chunk (for a 2-element
// lookahead)
template <typename T>
struct Halo2 {
...
// (deps/parlaylib-examples/suffix_array.h) off its hard-wired byte alphabet:
// the characters here are uint32 ranks (values in [1, n]; 0 is the past-end
// sentinel), so we bootstrap the sort on the first *two* uint32 characters
// packed into one 64-bit key (offset 2) instead of 12 bytes, then run the
// identical doubling loop — which only ever touches the integer `ranks`, never
// the characters, so it is alphabet-agnostic.  O(n log n) worst case, parallel;
// used so the base case saturates all cores rather than the old sequential
...
inline parlay::sequence<uint32_t> psa_int(const parlay::sequence<uint32_t>& S) {
  ...
  auto Clx = parlay::delayed_tabulate(
      n, [&](long i) { return std::pair{(s(i) << 32) | s(i + 1), (ulong)i}; });
  ...
  auto segs_from_flags = [&](parlay::sequence<bool>& fl, index seg_start) {
    ...
    auto segs = parlay::tabulate(
        offsets.size(),
        [&](long j) {
          index start = seg_start + (index)offsets[j];
          index end = seg_start + (index)((j == (long)offsets.size() - 1)
                                              ? fl.size()
                                              : offsets[j + 1]);
          ...
          parlay::parallel_for(
              start, end, [&](index i) { ranks[sorted[i]] = start + 1; },
              granularity);
  ...
  while (segments.size() > 0) {
    auto fl2 = parlay::map(
        ...
        [&](seg segment) {
          ...
          auto p = parlay::tabulate(
              l,
              [&](long i) {
                index k = sorted[st + i];
                return std::pair{
                    (k + offset >= n) ? (index)0 : ranks[k + offset], k};
              },
              granularity);
        ...
        1);
    segments = parlay::flatten(parlay::tabulate(
        segments.size(),
        [&](long i) { return segs_from_flags(fl2[i], segments[i].start); }, 1));
    offset = 2 * offset;
  }
  return sorted;
}
...
inline chunk_seq base_case(const chunk_seq& text, size_t n,
                           const std::string& out_prefix) {
  if (n == 1)
    return tabulate<uint32_t>(1, out_prefix,
                              [](size_t) { return uint32_t{0}; });
  parlay::sequence<uint32_t> s = read_u32(text, n);  // s[0..n-1]; past-end -> 0
  parlay::sequence<uint32_t> SA = psa_int(s);
  return tabulate<uint32_t>(n, out_prefix, [&SA](size_t i) { return SA[i]; });
}

// ── step 1: character triples for every sample R-index
// ────────────────────────
inline chunk_seq make_triples(const chunk_seq& text, size_t n, uint32_t n0,
                              uint32_t n1, const std::string& pfx) {
  ...
  chunk_seq trips = ExternalTransform<uint32_t, Triple>(
      text, pfx,
      [&pre, &halo, n, n0, nc](const uint32_t* in, size_t nl, size_t idx,
        ...
        if (fill > 0) {
          memset((char*)out + fill * sizeof(Triple), 0,
                 CHUNK_SIZE - fill * sizeof(Triple));
  ...
  if (n0 > n1) {
    chunk_seq dummy = tabulate<Triple>(
        1, pfx + "_dummy_", [n1](size_t) { return Triple{0, 0, 0, n1}; });
...
// ──── SA_R[g] = r (the g-th sample suffix in sorted order); srank of sample r
// is g + 1.  Emit a VAL record delivering that rank to pos_of_r(r).
inline chunk_seq scatter_vals(const chunk_seq& sa_r, uint32_t n0,
  ...
  return ExternalTransform<uint32_t, Join>(
      sa_r, pfx,
      [&pre, n0](const uint32_t* in, size_t nl, size_t idx,
        ...
        if (fill > 0) {
          memset((char*)out + fill * sizeof(Join), 0,
                 CHUNK_SIZE - fill * sizeof(Join));
          emit.emit(out, fill, idx * FAN + sub);
        } else {
          free(out);
        }
...
// ─────────────────────
inline chunk_seq make_base(const chunk_seq& text, size_t n,
                           const std::string& pfx) {
  auto pre = elem_prefix(text, sizeof(uint32_t));
  Halo2<uint32_t> halo = forward_halo2<uint32_t>(text);
  const size_t nc = text.chunks.size();
  ...
  const size_t FAN =
      (CHUNK_SIZE / sizeof(uint32_t) + cap - 1) / cap;  // in/out cap ratio
  return ExternalTransform<uint32_t, Join>(
      text, pfx,
      [&pre, &halo, n, nc, FAN](const uint32_t* in, size_t nl, size_t idx,
        ...
        for (size_t j = 0; j < nl; j++) {
          ...
          const size_t g = start + j;
          out[fill++] =
              Join{(uint32_t)g, /*slot=*/0, /*c0=*/in[j], /*c1=*/nb(g + 1)};
        }
        if (fill > 0) {
          memset((char*)out + fill * sizeof(Join), 0,
                 CHUNK_SIZE - fill * sizeof(Join));
...
// ──────────────
inline chunk_seq pair_pos(const chunk_seq& sorted_join,
                          const std::string& pfx) {
  ...
  return ExternalTransform<Join, Pos>(
      sorted_join, pfx,
      [nc, &first, &has_first](const Join* in, size_t nl, size_t idx,
        ...
        for (size_t i = 0; i < nl; i++) {
          ...
          if (i + 1 < nl) {
            nxt = in[i + 1];
            has_nxt = true;
          } else if (idx + 1 < nc && has_first[idx + 1]) {
            nxt = first[idx + 1];
            has_nxt = true;
          }
          const uint32_t self =
              (has_nxt && nxt.target == in[i].target && nxt.slot == 1) ? nxt.p0
                                                                       : 0;
          out[fill++] = Pos{in[i].target, in[i].p0, in[i].p1, self};
        }
        memset((char*)out + fill * sizeof(Pos), 0,
               CHUNK_SIZE - fill * sizeof(Pos));
...
// ─────────────────────────
inline chunk_seq make_sufrecs(const chunk_seq& pos_seq, size_t n,
                              const std::string& pfx) {
  ...
  return ExternalTransform<Pos, SufRec>(
      pos_seq, pfx,
      [&pre, &halo, n, nc](const Pos* in, size_t nl, size_t idx,
        ...
        for (size_t j = 0; j < nl; j++) {
          ...
          const size_t g = start + j;
          out[fill++] =
              SufRec{in[j].c0,       in[j].c1,    in[j].self, self_at(g + 1),
                     self_at(g + 2), (uint32_t)g, 0,          0};
        }
        if (fill > 0) {
          memset((char*)out + fill * sizeof(SufRec), 0,
                 CHUNK_SIZE - fill * sizeof(SufRec));
          emit.emit(out, fill, idx * FAN + sub);
        } else {
          free(out);
        }
...
inline chunk_seq extract_pos(const chunk_seq& sorted, const std::string& pfx) {
  return ExternalTransform<SufRec, uint32_t>(
      sorted, pfx,
      [](const SufRec* in, size_t nl, size_t idx,
         const ChunkEmitter<uint32_t>& emit) {
        uint32_t* out = emit.alloc();
        for (size_t i = 0; i < nl; i++) out[i] = in[i].pos;
        memset((char*)out + nl * sizeof(uint32_t), 0,
               CHUNK_SIZE - nl * sizeof(uint32_t));
        emit.emit(out, nl, idx);
      },
      /*max_out_per_input=*/1);
}

// ── extract a column (name) of a ByR-sorted (r,name) seq → R = names in
// r-order
inline chunk_seq extract_names(const chunk_seq& by_r, const std::string& pfx) {
  return ExternalTransform<ByR, uint32_t>(
      by_r, pfx,
      [](const ByR* in, size_t nl, size_t idx,
         const ChunkEmitter<uint32_t>& emit) {
        uint32_t* out = emit.alloc();
...
inline chunk_seq extract_r(const chunk_seq& by_name, const std::string& pfx) {
  return ExternalTransform<ByName, uint32_t>(
      by_name, pfx,
      [](const ByName* in, size_t nl, size_t idx,
         const ChunkEmitter<uint32_t>& emit) {
        uint32_t* out = emit.alloc();
...
// ────────────────
inline chunk_seq dc3_rec(const chunk_seq& text, size_t n,
                         const std::string& pfx) {
  ...
  chunk_seq sa_r;
  if (nm.groups == n02) {
    // Names are already a permutation of 1..n02: SA read off by sorting on
    // name.
  ...
  const std::string vp = pfx + "_val_", bp = pfx + "_base_",
                    jp = pfx + "_jsort_";
 ...
 * be < 2^32.
 */
inline chunk_seq ChunkDC3(const chunk_seq& text,
                          const std::string& out_prefix) {
  ...
  chunk_seq text_u32 = ExternalTransform<char, uint32_t>(
      text, tp,
      [](const char* in, size_t nl, size_t idx,
         const ChunkEmitter<uint32_t>& emit) {
        const size_t cap = emit.out_cap();
        ...
        do {
          ...
          memset((char*)out + cnt * sizeof(uint32_t), 0,
                 CHUNK_SIZE - cnt * sizeof(uint32_t));
