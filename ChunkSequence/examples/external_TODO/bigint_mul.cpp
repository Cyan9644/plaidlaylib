...
//
// Multiplies two n-limb (base-2^64, two's-complement) big integers stored
// across the SSDs using Karatsuba (see chunk_bigint_mul.h): chunk-aligned
// metadata cuts and shifts recurse the working set off the drives down to a
// DRAM base case, reusing the out-of-core big-integer ADD (chunk_bigint_add.h)
// at every level.
//
...
// karatsuba.h: upstream is broken (its signed add drops the carry-out when a
// cut low-half's top limb has the sign bit set, so it fails random schoolbook
// checks) — this mirrors how chunk_bigint_add.h ships its own reference.  The
// baseline doubles as a differential test (the out-of-core product is read back
// and compared value-for-value, exiting non-zero on any mismatch).  The signed
// path is covered by bigint_mul_test.  The baseline is gated by a RAM budget
// (half of physical RAM, overridable via EXAMPLE_INMEM_BUDGET_BYTES); past it
// it is skipped and the CSV field left blank.
//
...
//
// Usage: bigint_mulExample [global --flags] [n]      (n = number of 64-bit
// limbs)
...
int main(int argc, char* argv[]) {
  ...
  const std::string a_prefix = "bm_a", b_prefix = "bm_b",
                    prod_prefix = "bm_prod";

  std::cout << "Building two " << n << "-limb non-negative operands..."
            << std::flush;
  ...
  std::cout << result_limbs << " result limb(s)   " << std::setprecision(4)
            << mul_s << "s   " << std::setprecision(2) << gb_s
            << " GB/s (operands)\n";
  ...
  if (inmem_ok) {
    ...
    std::cout
        << "in-mem karatsuba (reference): skipped (operands exceed RAM budget "
  ...
  std::cout << "CSV," << n << ',' << f9(build_s) << ',' << f9(mul_s) << ','
            << (inmem_ok ? f9(inmem_mul_s) : std::string()) << ','
