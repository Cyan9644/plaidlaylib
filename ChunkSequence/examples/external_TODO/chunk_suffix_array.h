...
// examples/suffix_array.h) onto the chunk_seq data model. Upstream is a
// parallel prefix-doubling sort: sort suffixes by their first character, then
// repeatedly double the compared length by re-sorting on the pair (rank[p],
// rank[p+offset]). Upstream reads rank[p+offset] with a *random* gather into an
// in-DRAM array — exactly the access pattern the out-of-core library avoids.
// Here every such gather is turned into a **sort-join**, so the whole
// computation is a chain of streaming passes over the SSDs with no random
// access.
//
// Each doubling round (current sorted length = `offset`) is:
//
//   state PR = chunk_seq<(pos, rank)>            // rank of the suffix at pos
//   1. join-emit : for each (pos,rank) emit (pos, slot0, rank) and, if
//   pos>=offset,
//                  (pos-offset, slot1, rank).  The slot1 record delivers a
//                  suffix's rank to the position `offset` to its left.
//   2. sort      : direct_sample_sort by (a_pos, slot)  -> each position's
//   slot0
//                  is immediately followed by its slot1 (if any).
//   3. pair      : combine each position's two records into the doubling key
//                  SaKey(pos, k0=rank[pos], k1=rank[pos+offset] or 0).  The
//                  slot0/ slot1 pair can straddle a chunk boundary, resolved
//                  with a one-element forward halo taken from a per-chunk DRAM
//                  array (the first record of every chunk).
//   4. sort      : direct_sample_sort SaKey by (k0,k1).  THIS is the
//   duplicate-key-
//                  heavy sort (tied ranks); SaKey's ==/< compare (k0,k1) ONLY,
//                  with pos carried but uncompared, so direct_sample_sort's
//                  dedup assigner engages and the tied-key bucket run
//                  self-balances.  (A record that also compared pos would
//                  funnel a whole tied group into one bucket and overflow DRAM
//                  — verified separately.)
//   5. re-rank   : assign each element the sorted index of its (k0,k1) group
//   head
//                  (a two-level max-scan; the group-boundary test needs the
//                  previous element's key, supplied per chunk from a DRAM array
//                  of last-keys).  Emits the next round's PR = (pos, newrank)
//                  and the number of distinct groups.  When #groups == n every
//                  suffix has a unique rank and the sort is complete; the
//                  sorted-by-key order is the suffix array (its pos column).
//
...
struct JRec {
  ...
  bool operator==(const JRec& o) const {
    return a_pos == o.a_pos && slot == o.slot;
  }
  bool operator<(const JRec& o) const {
...
inline chunk_seq initial_keys(const chunk_seq& text, const std::string& pfx) {
  ...
  return ExternalTransform<char, SaKey>(
      text, pfx,
      [pre](const char* in, size_t n, size_t idx,
        ...
        do {
          ...
          memset((char*)out + cnt * sizeof(SaKey), 0,
                 CHUNK_SIZE - cnt * sizeof(SaKey));
...
// ────────
inline chunk_seq join_emit(const chunk_seq& prs, uint32_t offset,
  ...
  return ExternalTransform<PR, JRec>(
      prs, pfx,
      [offset](const PR* in, size_t n, size_t idx,
               const ChunkEmitter<JRec>& emit) {
        const size_t cap = emit.out_cap();
        JRec* out = emit.alloc();
        size_t fill = 0, sub = 0;
        ...
        for (size_t i = 0; i < n; i++) {
          ...
          if (in[i].pos >= offset)
            put(JRec{in[i].pos - offset, 1, in[i].rank, 0});
        }
        if (fill > 0) {
          memset((char*)out + fill * sizeof(JRec), 0,
                 CHUNK_SIZE - fill * sizeof(JRec));
...
inline chunk_seq pair_keys(const chunk_seq& sorted_j, const std::string& pfx) {
  ...
  return ExternalTransform<JRec, SaKey>(
      sorted_j, pfx,
      [nc, &first, &has_first](const JRec* in, size_t n, size_t idx,
        ...
        for (size_t i = 0; i < n; i++) {
          ...
          const uint32_t k1 =
              (has_nxt && nxt.a_pos == in[i].a_pos && nxt.slot == 1) ? nxt.rank
                                                                     : 0;
          out[fill++] = SaKey{in[i].rank, k1, in[i].a_pos, 0};
        }
        memset((char*)out + fill * sizeof(SaKey), 0,
               CHUNK_SIZE - fill * sizeof(SaKey));
...
// ───────── Thin wrapper over the shared assign_ranks: group by (k0,k1), carry
// pos.
inline RankResult assign_ranks(const chunk_seq& sorted_k,
                               const std::string& pfx) {
  return sa_detail::assign_ranks<SaKey>(sorted_k, pfx, sakey_same,
                                        [](const SaKey& k) { return k.pos; });
}
...
inline chunk_seq extract_sa(const chunk_seq& sorted_k, const std::string& pfx) {
  return ExternalTransform<SaKey, uint32_t>(
      sorted_k, pfx,
      [](const SaKey* in, size_t n, size_t idx,
         const ChunkEmitter<uint32_t>& emit) {
        uint32_t* out = emit.alloc();
        for (size_t i = 0; i < n; i++) out[i] = in[i].pos;
        memset((char*)out + n * sizeof(uint32_t), 0,
               CHUNK_SIZE - n * sizeof(uint32_t));
        emit.emit(out, n, idx);
      },
      /*max_out_per_input=*/1);
}
 ...
 */
inline chunk_seq ChunkSuffixArray(const chunk_seq& text,
  ...
  CHECK(rr.n == n) << "suffix_array: element count drift (" << rr.n << " vs "
                   << n << ")";

  uint32_t offset = 1;
  const size_t max_rounds =
      64;  // >= ceil(log2 n) + slack; guards infinite loop
  for (size_t stage = 1; rr.groups < n; stage++) {
    ...
    const std::string jp = sp("j", stage), j2p = sp("js", stage),
                      skp = sp("s", stage);
