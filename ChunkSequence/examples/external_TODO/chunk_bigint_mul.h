...
// Self-contained (no upstream include): used for the recursion's DRAM base
// case.
...
inline std::vector<digit> in_mem_karatsuba(const std::vector<digit>& a,
  ...
  const std::vector<digit> a0(a.begin(), a.begin() + m),
      a1(a.begin() + m, a.end());
  const std::vector<digit> b0(b.begin(), b.begin() + m),
      b1(b.begin() + m, b.end());
  ...
  std::vector<digit> r(
      std::max(z0.size(), std::max(zm.size() + m, z2.size() + 2 * m)) + 1, 0);
...
inline size_t default_dram_budget() {
  ...
  if (const char* e = getenv("BIGINT_MUL_DRAM_BUDGET_BYTES"))
    b = std::stoull(e);
  return std::max<size_t>(b, CHUNK_SIZE * 4);
}

// Scratch bookkeeping: every op that writes files gets a unique prefix,
// recorded so the top-level call can unlink all of them at the end.  fresh() is
// called from parallel recursion branches, so it is thread-safe.
struct Ctx {
  size_t budget;
  std::string base;
...
// A big integer is stored as a canonical non-negative magnitude iff its top
// limb has the sign bit clear.  If not, attach a zero guard limb so
// ChunkBigIntAdd treats it as unsigned.  Only called on chunk-aligned cut
// halves (all chunks full), so the metadata append keeps the dense-except-last
// invariant.
...
inline chunk_seq subtract(Ctx& ctx, const chunk_seq& a, const chunk_seq& b) {
  return ChunkBigIntAdd(a, complement(ctx, b), ctx.fresh("sub"),
                        /*extra_one=*/true);
}
...
inline chunk_seq karatsuba(Ctx& ctx, const chunk_seq& a, const chunk_seq& b) {
  ...
  if (nb > m) {
    ...
    // z2*B^(2m) + zm*B^m + z0.
    chunk_seq lo =
        ChunkBigIntAdd(prepend_zero_chunks(zm, split), z0, ctx.fresh("lo"));
    return ChunkBigIntAdd(prepend_zero_chunks(z2, 2 * split), lo,
                          ctx.fresh("r"));
  }
...
// Karatsuba down to a DRAM base case, and returns a fresh index-ordered
// chunk_seq owned under result_prefix (all scratch files are removed before
// returning).
inline chunk_seq ChunkBigIntMul(const chunk_seq& a, const chunk_seq& b,
  ...
  // then drop every scratch file (peak scratch disk = sum of live
  // intermediates; an accepted research-example trade-off, cf.
  // bigint_add_eager's unlink).
  chunk_seq out =
      from_chunks(parlay::sequence<chunk>(mag.chunks.begin(), mag.chunks.end()),
                  result_prefix);
  cleanup(ctx);
  return out;
}
