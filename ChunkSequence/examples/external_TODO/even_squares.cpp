...
//                                        filter+map+reduce
//                                        (parlay::delayed::filter
//                                        + map, two delayed stages fused at
//                                        reduce).
...
#include "ChunkSequence/examples/external_TODO/external_even_squares.h"
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Building " << n << "-element out-of-core input..."
            << std::flush;
  ...
  bench_drives::settle_drives();  // isolate the first timed op from the build's
                                  // writeback
  ...
  size_t inmem_eager_result = 0, inmem_delay_result = 0,
         inmem_actually_delay_result = 0;
  double inmem_eager_op_s = 0, inmem_delay_op_s = 0,
         inmem_actually_delay_op_s = 0;
  if (inmem_ok) {
    std::cout << "Building " << n << "-element in-memory input..."
              << std::flush;
    ...
    std::cout << "Running in-memory actually-delayed (filter_op)..."
              << std::flush;
    t0 = Clock::now();
  ...
  auto check = [&](const std::string& name, size_t got, size_t expected) {
    if (got != expected) {
      std::cout << "*** MISMATCH (" << name << "): got " << got << ", expected "
                << expected << " ***\n";
      agree = false;
    }
  };
  ...
  if (inmem_ok) {
    ...
    check("in-mem actually-delayed vs eager", inmem_actually_delay_result,
          eager_result);
  }
  ...
  std::cout << "CSV," << n << ',' << f9(build_s) << ',' << f9(eager_op_s) << ','
            ...
            << eager_result << ',' << f9(eager_gb_s) << ',' << f9(delay_gb_s)
            << '\n';
