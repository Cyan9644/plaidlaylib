...
// GetFileName(prefix, d), because our sort's recursion leaves
// tag/bucket-suffixed intermediates (ss_id_<tag><slot>,
...
// and the per-bucket base sorter's qs_base_ output (the sorted result flatten
// returns *is* the qs_base_ files, so this prefix must be swept or the entire
// output leaks on the drives every point).  Peter's: pss_in/pss_out + his
