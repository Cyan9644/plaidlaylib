...
//
// external_bellman_ford (chunk_bellman_ford.h) is a port of the "pull"
// variant of parlaylib's bellman_ford, byte-identical to
// deps/parlaylib-examples/bellman_ford.h: each round, relax every vertex's
// distance from its in-neighbors' chunk_csr row.
...
// SequentialReadContext -- see chunk_bellman_ford.h), so its cost scales
...
#include "ChunkSequence/helper/graph_utils/graph_utils.h"
#include "parlaylib-examples/bellman_ford.h"
#include "utils/bench_drives.h"
#include "utils/file_utils.h"
#include "utils/trace_marker.h"
// Before chunk_bellman_ford.h: this pulls in direct_samplesort, which must be
// parsed before external_compressed_sparse_row.h's bare macros.
...
static bool run_case(const std::string& label, size_t n_req,
  ...
  // reader setup -- see chunk_bellman_ford.h for why that's expected to
  ...
  // an n^2/2-scaling "dense" case) on every one of them, not just the
  // largest.  Default matches the size at which that DRAM cost stops being a
  // useful baseline point; raise BELLMAN_FORD_INMEM_MAX_N on a box with
  // enough DRAM to still want the comparison at larger n (the baseline runs
