...
//
// Computes every prime in [0, n) with data stored across the SSDs.  Small
// primes (up to sqrt(n)) are sieved in memory, then each 4 MB virtual chunk
// [start, end) is sieved independently in parallel and its survivors collected;
// ChunkFlatTabulate packs the variable-length per-chunk survivor lists into a
// dense output chunk_seq.
//
...
// examples sweep (make bench-examples) times chunk_primes() across a sweep of
// n.
//
...
// in-memory comparison baseline; the out-of-core sieve stays self-contained
// on its own in_mem_primes (chunk_primes.h).  Defines global primes(long) —
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "pi(" << n << ") = " << count << "   " << std::fixed
            << std::setprecision(4) << secs << "s   " << std::setprecision(2)
            << to_gb(out_bytes) / secs << " GB/s (eff. output)\n";
  ...
  if (inmem_ok) {
    ...
    std::cout << "in-mem parlaylib sieve: pi(" << n
              << ") = " << primes_mem.size() << "   " << std::setprecision(4)
              << inmem_secs << "s\n";
  ...
  // Read the last output chunk and print the final few primes (cheap: one
  // chunk).
  if (count > 0) {
  ...
  cleanup_prefix(
      prefix);  // don't leave the output on the drives across sweep points
  return agree ? 0 : 1;
}
