...
chunk_seq add(const chunk_seq& a, const chunk_seq& b, bool extra_one) {
  ...
  auto carry_fn = [](enum carry a, enum carry b) {
    return (b == propagate) ? a : b;
  };
  ...
  auto result = (delayed::map(triple_zipped, [&](auto triple) {
    ...
    // bring in the extra one if needed (c can only be propagate at the least
    // significant digit I think)
