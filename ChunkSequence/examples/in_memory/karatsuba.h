...
auto shift(const bigint& a, int n) {
  return parlay::delayed::tabulate(
      a.size() + n, [&, n](long i) { return (i < n) ? 0 : a[i - n]; });
}
...
bigint karatsuba(const Bigint& a, const Bigint& b) {
  ...
  long nhalf = nb / 2;
  auto low_a = a.cut(0, nhalf);
  auto high_a = a.cut(nhalf, na);
  auto low_b = b.cut(0, nhalf);
  auto high_b = b.cut(nhalf, nb);
  bigint z0, z1, z2;
  parlay::par_do3(
