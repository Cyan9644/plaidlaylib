...
auto bucketed_dijkstra(vertex start, const weighted_graph& G) {
  ...
  auto distances = parlay::tabulate<std::atomic<int>>(
      G.size(), [&](long i) { return (i == start) ? 0 : G.size(); });
  ...
  while (d <= max_distance) {
    ...
    auto frontier =
        filter(flatten(buckets[d]), [&](auto v) { return distances[v] == d; });
    ...
    if (frontier.size() > 0) {
      ...
      auto edges = delayed::flatten(parlay::map(frontier, [&](vertex u) {
        return delayed::map(
            G[u], [=](auto v) { return std::pair{d + v.second, v.first}; });
      }));
      ...
      if (keep.size() > 0) {
        ...
        int max_d =
            parlay::reduce(delayed::map(keep, [](auto p) { return p.first; }),
                           parlay::maximum<int>());
