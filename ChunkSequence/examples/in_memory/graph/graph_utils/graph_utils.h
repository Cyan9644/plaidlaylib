...
// (deps/parlaylib-examples/helper/graph_utils.h): an include guard.  Upstream
// has none, so this header could only ever be included once per translation
// unit; external_rmat.h needs rmat_edge too, and bellman_ford.cpp includes
// both.
...
struct graph_utils {
  ...
  static graph symmetrize(const edges& Ein, long n) {
    ...
    return parlay::tabulate(
        n, [&](long i) { return remove_duplicates(append(G[i], GT[i])); });
  }
