  ...
  for (int i = 0; i < n; i++) {
    auto dn = parlay::map(GT, [&](auto& ngh) {
      return parlay::reduce(
          parlay::delayed_map(ngh,
                              [&](auto e) { return d[e.first] + e.second; }),
          parlay::minimum<wtype>());
    });
    dn[start] = 0.0;
...
auto bellman_ford_lazy(vertex start, const weighted_graph& G,
  ...
  auto visited = parlay::tabulate<std::atomic<bool>>(
      n, [&](long i) { return (i == start) ? true : false; });

  // distances used when lazy (need to be atomic)
  auto da = parlay::tabulate<std::atomic<wtype>>(
      n, [&](long i) { return (i == start) ? 0.0 : max_d; });
  ...
  while (active.size() > 0 && ++cnt <= n) {
    ...
    if (do_lazy) {
      ...
      if (!lazy) parlay::parallel_for(0, n, [&](long i) { da[i] = d[i]; });
      lazy = true;
    ...
    // *** use greedy if number of active vertices is large enough
      ...
      if (lazy) parlay::parallel_for(0, n, [&](long i) { d[i] = da[i]; });
      lazy = false;
      ...
      active = parlay::pack_index<vertex>(
          parlay::delayed_tabulate(n, [&](long i) { return dn[i] != d[i]; }));
      d = std::move(dn);
    }
  }
