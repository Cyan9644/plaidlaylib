...
#include <functional>
#include <random>
// parlay::internal::heap_tree comes in via <parlay/primitives.h>; its header
// has no include guard, so do NOT include <parlay/internal/heap_tree.h> again
// here.
...
auto kth_smallest(Range&& in, long k, Less less = {}) {
  ...
  auto pivots = parlay::sort(parlay::tabulate(sample_size * over,
                                              [&](long i) {
                                                auto r = gen[i];
                                                return in[dis(r)];
                                              }),
  ...
  parlay::internal::heap_tree ss(pivots);
  auto ids = parlay::tabulate(
      n, [&](long i) -> unsigned char { return ss.rank(in[i], less); });
  ...
  auto next = parlay::pack(
      in, parlay::delayed_map(ids, [=](auto b) { return b == id; }));
