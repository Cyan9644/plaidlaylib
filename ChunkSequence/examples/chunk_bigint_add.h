...
// in-memory parlaylib reference used as the example's baseline and as the
// tests' differential oracle.  Big integers are two's-complement, little-endian
...
// block reduce to `no`, killing the carry — wrong for e.g. 1 + (all ones),
// where every block past the first is all-propagate.  Both implementations
// below use `propagate` as the identity and resolve any residual `propagate` in
// the output (a carry that propagated unbroken from the initial carry-in) to
// `extra_one`. (Earlier drafts of this code used `no`/`extra_one` as the
// identity — a real correctness bug this header fixes.)
...
inline carry classify(double_digit a_val, double_digit b_val,
                      double_digit mask) {
...
// ───────────────────── A deliberately un-fused ("eager") counterpart to
// ChunkBigIntAdd, used only by the bigint_add_eager benchmark to show the I/O
// cost the delayed layer avoids. The library has no eager two-input combine, so
// the two zips necessarily stay delayed; but the intermediate map(classify) and
// scan(carry) are materialized to disk between primitives instead of being
// fused into one pass:
...
// prologue and overflow epilogue as ChunkBigIntAdd; produces an identical
// result.
inline chunk_seq ChunkBigIntAddEager(const chunk_seq& a, const chunk_seq& b,
  ...
  chunk_seq classes =
      delayed::force(delayed::map(pairs,
                                  [mask](auto p) {
                                    return (digit)classify(
                                        std::get<0>(p), std::get<1>(p), mask);
                                  }),
                     cls_prefix);
...
bigint add(const Bigint1& a, const Bigint2& b, bool extra_one = false) {
  ...
  auto B = parlay::delayed::tabulate(
      na, [&](size_t i) { return (i < (size_t)nb) ? b[i] : pad; });
