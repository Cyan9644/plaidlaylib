...
//
// Builds an n-point cloud across the SSDs (pseudo-random coordinates in the
// unit square, deterministic from parlay::hash64), then finds the upper hull
// with the out-of-core quickhull: a ChunkReduce for the farthest point and two
...
// half of physical RAM, override via EXAMPLE_INMEM_BUDGET_BYTES; when skipped
// the CSV field is left blank so the plotted in-mem line stops at the RAM
// cliff.
//
...
// computable anywhere.  Random doubles ⇒ no three points collinear and no
// coordinate ties (both probability zero), so the strict-area hull is
// unambiguous.
...
int main(int argc, char* argv[]) {
  ...
  // the region is split with the streaming out-of-core
  // ChunkReduce+ChunkPartition.
  //
  // Keep this SMALL, not a big fraction of RAM.  For a discard-heavy problem
  // like the hull, one streaming partition pass drops almost all interior
  // points at ~device read speed, whereas the in-memory quickhull on a huge
  // region is slow: every recursion level allocates and copies fresh parlay
  // sequences over (here) billions of points.  So we want the out-of-core
  // partition to do the bulk discarding and hand qh_mem only a small residual.
  // A few GiB (≈1e8 points, an in-memory quickhull of ~1 s) is the sweet spot;
  // the extra out-of-core levels are cheap because each streams a rapidly
  // shrinking region.
  ...
  std::cout << count << " hull vertices   " << std::setprecision(4) << hull_s
            ...
            << " GB/s (points read)   "
            << "out-of-core split levels: " << plaid::last_ext_splits() << "\n";

  // In-memory baseline: parlaylib's upper_hull on the same points,
  // cross-checked for an identical hull (same indices, same order).
  bool agree = true;
