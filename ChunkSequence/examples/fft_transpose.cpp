...
// between the two passes, so both length-A and length-B FFT passes are contiguous
// streaming.  This is the counterpart to fftExample (which skips the transpose and
// does the length-B pass by random 4 KiB gather/scatter): the two binaries let you
// measure the classic external-memory tradeoff on SSDs directly --
//
//   fftExample (transpose-free): stage1 (2N) + stage2 random (2N) = 4N, half random
//   fft_transposeExample (this): stage1 (2N) + transpose (2N) + stage2T (2N) = 6N,
//                                all streaming.
//
// On HDDs the sequential 6N wins hugely; on SSDs the 4N-with-random path may win.
// The three stages are separately timed and marked for benchmarks/io_trace.py.  See
// examples/chunk_fft.h for the algorithm, the transpose, and the four-step math.
//
// Dual-purpose like the other examples: prints human-readable output AND a "CSV,"
// line run_benches.py greps.  When it fits RAM it times the SAME in-memory
// four-step baseline as fftExample (inmem_s, for the make bench-examples plot);
// FFT_VERIFY=1 additionally cross-checks the full spectrum against the independent
// parlaylib complex_fft oracle (using the transpose variant's output permutation)
// and exits non-zero on a mismatch.  Both are skipped under PLAID_TRACE and gated
// by a RAM budget of half physical RAM (EXAMPLE_INMEM_BUDGET_BYTES, 0 disables).
//
//   usage: fft_transposeExample [global --flags] [n]   (FFT_VERIFY=1 to cross-check)
//     n   sequence length (default 1<<20); power of two, 2^16 <= N <= 2^36 (B <= EPC).
//
// CSV line:
//   CSV,<N>,<build_s>,<stage1_s>,<transpose_s>,<stage2t_s>,<total_s>,<inmem_s>,<count>,<gb_s>
//   total_s = stage1_s + transpose_s + stage2t_s; throughput = 6*N*16 / total_s.
...
// Upstream parlaylib example (fetched by `make deps`): the independent complex_fft
// oracle used only under FFT_VERIFY.  Global symbols, no include guard -- the only
// upstream header in this TU.
...
// Deterministic logical input value at logical index m (identical to fftExample).
...
int main(int argc, char* argv[]) {
    ...
    CHECK(B <= ChunkFFT::EPC) << "transpose variant requires B <= EPC (N <= 2^36); "
    ...
    auto t0 = Clock::now();
    chunk_seq input = ChunkSequenceOps::tabulate<cd>(N, in_prefix, [A, B](size_t p) {
        const size_t a = p % A;
        const size_t b = p / A;
    ...
    // Transpose: materialize the A x B transpose on disk (streaming band transpose).
    ...
    const size_t bytes_moved = 6 * N * sizeof(cd);   // stage1 + transpose + stage2t, r+w each
    ...
    // In-memory baseline: the SAME four-step in DRAM (identical to fftExample), and
    // under FFT_VERIFY the independent complex_fft oracle over the full spectrum.
    bool agree = true;
    double inmem_s = 0;
    if (do_baseline) {
        ...
        inmem_s = elapsed(t0);
        std::cout << "in-mem four-step FFT: " << std::setprecision(4) << inmem_s << "s";

        if (do_verify) {
            auto Xref = complex_fft(x);
            std::vector<cd> out = s2.to_vector<cd>();
            auto e = ChunkFFT::spectrum_errs(out, Xmem, Xref, d, ChunkFFT::out_perm_transpose);
    ...
    std::cout << "CSV," << N << ',' << f9(build_s) << ',' << f9(stage1_s) << ','
              ...
              << (do_baseline ? f9(inmem_s) : std::string()) << ','
              << N << ',' << f9(gb_s) << '\n';
