...
// bench-examples plot); like fftExample (and unlike the frozen pre-cleanup
// driver's FFT_VERIFY=1 opt-in), the full-spectrum cross-check against the
// independent parlaylib complex_fft oracle (using the transpose variant's
// output permutation) now runs by default whenever the baseline runs, and
// exits non-zero on a mismatch.  Both are skipped under PLAID_TRACE and gated
// by a RAM budget of half physical RAM (EXAMPLE_INMEM_BUDGET_BYTES, 0
// disables).
//
//   usage: fft_transposeExample [global --flags] [n]
...
// complex_fft oracle used for the cross-check.  Global symbols, no include
...
int main(int argc, char* argv[]) {
  ...
  // and (when it runs) the independent complex_fft oracle over the full
  // spectrum.
  bool agree = true;
  double inmem_s = 0;
  if (do_baseline) {
