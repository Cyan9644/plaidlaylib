...
//
// This is a *port of Peter's sample sort* (peter_samplesort/:
// peter_samplesort.h
...
// writev()s, the two writer I/O threads, the per-worker read/sort/write
// pipeline in the gather phase -- so that a head-to-head run
...
// one reads a chunk_seq through ChunkSequenceReader and names its output
// chunks.
//
// external_samplesort.h is the other experiment: the same algorithm built out
// of the library's primitives (delayed map -> count_sort -> ... -> flatten).
// Between the three, "algorithm", "substrate" and "primitives overhead" are
// each isolated.  This file is deliberately an example, not a library
// primitive: what it does by hand is the list of abstractions the primitives
// are missing (see the end of this comment).
//
...
//            The writer coalesces those fragments into >=1 MiB writev()s
//            appended to one file per bucket, on two io_uring threads.
...
//            DRAM, write the previous one back out (io_uring) -- so reads,
//            sorts and writes of different buckets overlap on every worker.
//
// The output chunk_seq is the phase-2 result files, carved at CHUNK_SIZE
// offsets: a bucket is written as one contiguous file, so chunk j of bucket b
// is just [j*CHUNK_SIZE, min((j+1)*CHUNK_SIZE, bucket_bytes)) in that file, and
// no data is copied to produce it.  As in external_samplesort (whose flatten()
// does the same), the last chunk of each bucket is partial, so the result is
// index-ordered but not densely packed.
//
...
//      repeat, which is exactly what the deduplicating assigner already
//      handles.
//   3. His writer I/O threads spin on a zero-timeout queue poll when idle;
//   these
//      block instead.  Same requests, same order, two fewer cores burned.
//   4. Ring creation goes through InitIoUringWithRetry (io_uring's
//   RLIMIT_MEMLOCK
//      accounting is reclaimed asynchronously on the WSL2 dev kernel, so a
//      burst of ring churn transiently fails with ENOMEM).
...
// still lands in one bucket.  The sort is single-level: buckets are sized to
// fit in DRAM and sorted there, with no recursion.
//
...
//     worker before it can write (num_workers * num_buckets * CHUNK_SIZE of
//     DRAM, which forces small buckets and extra recursion).  Here a worker's
//     per-bucket buffer is 4 KiB and the *writer* rebuilds large sequential
//     writes out of them with iovecs -- the decoupling Peter's
//     OrderedFileWriter does.
...
// bucketed_file_writer.h (BucketWriter/bucket_allocator/BucketData),
// chunk_seq_reader.h (ChunkSequenceReader) and utils/simple_queue.h
// (SimpleQueue) have all since been merged into chunk_seq.h.
...
// ── Peter's constants
// ───────────────────────────────────────────────────────── configs.h already
// carries his SAMPLE_SORT_BUCKET_SIZE (4 KiB, the per-worker per-bucket scatter
// buffer), IO_VECTOR_SIZE (1024, iovecs per writev) and MAIN_MEMORY_SIZE (the
// DRAM budget the bucket count is derived from).  The rest are inlined in his
// code:
constexpr size_t kTargetBucketBytes = 1UL
                                      << 27;  // GetSampleSize: ~128 MB/bucket
...
// UnorderedFileReader defaults (2 threads x 64 in-flight 512 KiB reads) keep
// 128 reads outstanding, enough to cover every drive several times over.  The
// chunk grid fixes our read size at CHUNK_SIZE instead, so the same coverage is
// bought with more threads and fewer reads each: 10 x 8 = 80 outstanding, >
// SSD_COUNT.
constexpr size_t kReaderThreads = 10;
constexpr size_t kReaderQueueDepth = 32;
constexpr size_t kReaderMaxInFlight = 8;
constexpr size_t kReaderQueueSize =
    128;  // CHUNK_SIZE buffers held by the reader
...
// stand-in for FileInfo::before_size.
...
// requests are split into one segment per worker, and each worker drives its
// own io_uring (512 deep) over its segment, so the sampling reads are all in
// flight at once rather than one blocking pread at a time — which matters at
// scale, where the oversample is num_pivots^1.5 random blocks.
template <typename T>
parlay::sequence<T> RandomBatchRead(const chunk_seq& seq,
  ...
  auto out = parlay::flatten(parlay::map(
      parlay::iota(num_threads),
      [&](size_t segment) {
        const size_t start = segment_size * segment;
        const size_t end =
            std::min(segment_size * (segment + 1), requests.size());
        if (end <= start) return parlay::sequence<T>();

        const size_t num_buffers = std::min(kSampleBuffers, end - start);
        auto* buffers = (ReadRequest*)std::aligned_alloc(
            O_DIRECT_MEMORY_ALIGNMENT, sizeof(ReadRequest) * num_buffers);
        CHECK(buffers != nullptr)
            << "direct_sample_sort: sample buffer alloc failed";
        ...
        while (i < end || in_ring > 0) {
          ...
          while (i < end && !free_buffers.empty() &&
            ...
            const size_t ci =
                (size_t)(std::upper_bound(prefix.begin(), prefix.end(), idx) -
                         prefix.begin()) -
                1;
...
inline size_t GetSampleSize(size_t total_bytes, size_t n) {
  ...
  const size_t min_samples = std::max<size_t>(
  ...
  return std::max(std::min(total_bytes / kTargetBucketBytes, max_samples),
                  min_samples);
}
...
parlay::sequence<T> GetPivots(const chunk_seq& seq,
                              const std::vector<size_t>& prefix, size_t n,
                              size_t num_pivots, Less less) {
  // Too many samples means the buckets are distributed too evenly, which costs
  // more than it buys.
  const size_t oversample = std::max<size_t>(
      1, std::min(n, num_pivots * (size_t)std::sqrt(num_pivots)));

  parlay::random_generator generator;
  std::uniform_int_distribution<size_t> dis(0, n - 1);
  auto samples =
      parlay::sort(RandomBatchRead<T>(seq, prefix,
                                      parlay::map(parlay::iota(oversample),
                                                  [&](size_t i) {
  ...
  for (size_t remaining = num_pivots; remaining > 0; remaining--) {
    const size_t left =
        (oversample > i + remaining) ? oversample - i - remaining : 0;
...
template <typename T, typename Less>
inline size_t BinarySearch(const parlay::sequence<T>& pivots, const T& t,
                           Less less) {
...
// copy of it to the same bucket would overflow that bucket's DRAM, so the
// copies are spread randomly across the run.  Every other key goes through the
// binary search.  The map is empty when the pivots are distinct.
template <typename T, typename Less>
class DeduplicatingAssigner {
 public:
  DeduplicatingAssigner(parlay::sequence<T> pivots, Less less)
    ...
    if (count > 1)
      map_.emplace(prev,
                   std::make_pair((uint32_t)pivots_.size() - count, count));
  }
 ...
 * files and the gather phase's final sorted output -- across `disk_span`
 ...
 * one-file-per-bucket layout, which throttles reading any contiguous slice
 * of the output (e.g. its lowest-key bucket) to a single drive; passing
 * GetSSDList().size() spreads every bucket's reads and writes across every
 * drive instead.
 */
...
chunk_seq direct_sample_sort(const chunk_seq& seq, Less less = {},
    ...
    // at once (CPUs idle).  At disk_span==1 (the default) each stage reads/sorts
    // /writes a single buffer in place, exactly as before, and peak DRAM is ~3
    // buckets per worker.  At disk_span>1 a bucket's disk_span shards are read
    // into separate buffers, compacted into one merged buffer for the sort (the
    // shards' padding must not land in the middle of the sorted range), then
    // re-split into disk_span aligned output buffers before writing -- so
    // `process` below briefly holds both the merged buffer and every output
    // shard at once, pushing peak DRAM to ~4 buckets per worker.
    std::vector<size_t> ids;   // logical buckets with data, in pivot order
    for (size_t b = 0; b < num_buckets; b++) {
        bool has_data = false;
    ...
    // Output shard filenames are set up front; true_bytes/file_bytes are
    // filled in during the gather phase below, since an output shard is a
    // fresh split of the merged, sorted bucket -- not a copy of any one
    ...
    parlay::parallel_for(0, parlay::num_workers(), [&](size_t) {
        ...
        while (submit_read || reap_write) {
            ...
            // compact the bucket's shards (if any), sort, split into disk_span
            // output shards, and open their result files
            if (process) {
                auto t0 = std::chrono::steady_clock::now();
                ...
                if (disk_span == 1) {
                    ...
                    // place and reuse it directly as the write buffer, exactly
                    // as before disk_span existed -- no extra copy.
                    const auto& r = buckets[b];
                    const size_t nelem = r.true_bytes / sizeof(T);
                    T* buffer = current.read_bufs[0];
                    current.read_bufs[0] = nullptr;
                    parlay::sort_inplace(parlay::make_slice(buffer, buffer + nelem), less);
                    ...
                    CHECK(merged != nullptr) << "direct_sample_sort: merge alloc failed ("
                    ...
                    parlay::sort_inplace(parlay::make_slice(merged, merged + nelem), less);
                    ...
                    for (size_t s = 0; s < disk_span; s++) {
                        ...
                        CHECK(current.write_bufs[s] != nullptr)
                            << "direct_sample_sort: output shard alloc failed (" << file_b
                            << " bytes)";
    ...
    // shard-minor (index b*disk_span+s), so a linear scan concatenates every
    // bucket's shards in shard order, then buckets in pivot order, giving the
    // sorted seq (a no-op nesting at disk_span==1, one shard per bucket).
    chunk_seq out;
    size_t index = 0;
