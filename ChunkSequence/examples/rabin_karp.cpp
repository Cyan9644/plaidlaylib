...
//
// CSV line:
// CSV,<n>,<m>,<build_s>,<search_s>,<inmem_search_s>,<count>,<throughput_gb_s>
//   throughput = text bytes read / search_s (search is one streaming read
//   pass).
//
...
// (just the following bytes of the same contiguous buffer).  RkScanChunk
// reports matches starting in [0, len), so blocks partition the text and each
// start is scanned exactly once; flatten concatenates the per-block lists in
// text order.
static parlay::sequence<uint64_t> in_mem_rabin_karp(
    const parlay::sequence<char>& text, const parlay::sequence<char>& pattern) {
  namespace d = plaid::detail;
  ...
  auto per_block = parlay::tabulate(nb_blocks, [&](long b) {
    ...
    d::RkScanChunk<char>(text.data() + start, len, text.data() + start + len,
                         ov, pattern.data(), m, ph, x, x_m1, (uint64_t)start,
                         out);
    return out;
  });
...
int main(int argc, char* argv[]) {
  ...
  trace_mark("op_start");
  t0 = Clock::now();
  chunk_seq matches = plaid::ChunkRabinKarp<char>(text, out_prefix, pattern);
  ...
  std::cout << count << " match(es)   " << std::setprecision(4) << search_s
            << "s   " << std::setprecision(2) << gb_s << " GB/s (text read)\n";
  ...
  if (inmem_ok) {
    ...
    std::cout << "in-mem rolling-hash Rabin-Karp: " << matches_mem.size()
              << " match(es)   " << std::setprecision(4) << inmem_search_s
              << "s\n";
  ...
  std::cout << "CSV," << n << ',' << m << ',' << f9(build_s) << ','
            << f9(search_s) << ','
            << (inmem_ok ? f9(inmem_search_s) : std::string()) << ',' << count
            << ',' << f9(gb_s) << '\n';
