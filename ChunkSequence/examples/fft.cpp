...
// length-A pass over contiguous blocks (ExternalTransform) and a random length-B
// pass over strided columns (small io_uring 4 KiB gather/scatter).  See
...
// transpose entirely.  The two stages are separately timed and separately marked
// for benchmarks/io_trace.py so the streaming vs random I/O contrast is visible.
//
// Dual-purpose like the other examples: prints human-readable output AND a
// machine-readable "CSV," line that benchmarks/run_benches.py greps.  When it fits
// RAM it times an in-memory baseline (the inmem_s the `make bench-examples`
// comparison plot needs) -- and that baseline is the SAME transpose-free four-step
// FFT run in DRAM (same fft_inplace kernel and parallel structure as the
// out-of-core stages), so the comparison isolates the I/O cost rather than pitting
// our algorithm against a different FFT implementation.  The full-spectrum
// correctness cross-check against an independent oracle (parlaylib complex_fft) --
// reading the entire result back and comparing every bin -- is an extra heavy pass
// (FFT's output is as large as its input), so it is opt-in via FFT_VERIFY=1 and
// exits non-zero on a mismatch.  Both the baseline and the compare are skipped
// under PLAID_TRACE (they must not inflate a trace) and are gated by a RAM budget
// of half physical RAM; override via EXAMPLE_INMEM_BUDGET_BYTES (0 disables both).
//
//   usage: fftExample [global --flags] [n]        (FFT_VERIFY=1 to cross-check)
//     n   sequence length (default 1<<20); rounded down to a power of two, >= 2^16.
//
// CSV line:
...
// Deterministic logical input value at logical index m: pseudo-random complex in
...
int main(int argc, char* argv[]) {
    ...
    // The in-mem parlaylib complex_fft plays two separable roles:
    ...
    //     all N bins) -- heavy (its output is as big as its input) and only useful
    //     as a differential test, so opt-in via FFT_VERIFY=1.
    // Both are skipped under PLAID_TRACE (must not inflate a trace) and both are
    // RAM-budget gated; set EXAMPLE_INMEM_BUDGET_BYTES=0 to force the baseline off.
    ...
    if (do_baseline) {
        ...
        // DRAM array with no I/O.  The column-major placement is the untimed "build"
        // equivalent; only the two transform passes are timed, matching the
