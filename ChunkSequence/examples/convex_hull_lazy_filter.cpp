...
//
// CSV line: CSV,<n>,<build_s>,<hull_s>,<lazyfilter_hull_s>,<inmem_hull_s>,<count>,<throughput_gb_s>
//   hull_s/lazyfilter_hull_s: ChunkPartition-based / lazy_filter-based hull pass.
//   throughput = point bytes read / hull_s (ChunkPartition series only).
...
// anywhere. Identical to convex_hull.cpp's generator, so both drivers put the
// same points at the same index.
...
int main(int argc, char* argv[]) {
    ...
    std::vector<uint64_t> hull_lazy =
        ChunkSequenceOps::UpperHullLazyFilter(points, dram_budget, "ch_lazy_scratch");
    const double lazyfilter_hull_s = elapsed(t0);
    std::cout << " done\n";
    std::cout << hull_lazy.size() << " hull vertices   "
              << std::setprecision(4) << lazyfilter_hull_s << "s   "
              << "out-of-core split levels: " << ChunkSequenceOps::last_ext_splits()
              << "\n";

    // In-memory baseline: parlaylib's upper_hull on the same points, cross-checked
    // for an identical hull (same indices, same order) against BOTH out-of-core
    // algorithms.
    bool agree = true;
    ...
    if (inmem_ok) {
        ...
        auto check = [&](const char* label, const std::vector<uint64_t>& h) {
            if (hull_mem.size() != h.size()) {
                std::cout << "*** MISMATCH (" << label << "): in-mem count " << hull_mem.size()
                          << " != out-of-core count " << h.size() << " ***\n";
                agree = false;
                return;
            }
    ...
    if (hull.size() != hull_lazy.size()) {
        ...
        for (size_t i = 0; i < hull.size(); i++)
            if (hull[i] != hull_lazy[i]) {
                std::cout << "*** MISMATCH: ChunkPartition vs lazy_filter differ at vertex "
                          << i << " ***\n";
                agree = false;
                break;
            }
    ...
    // Columns: n,build_s,hull_s,lazyfilter_hull_s,inmem_hull_s,count,throughput_gb_s
