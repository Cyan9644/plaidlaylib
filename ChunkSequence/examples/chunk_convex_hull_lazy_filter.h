...
chunk_seq materialize_wide(const D& d, const std::string& result_prefix) {
    ...
    constexpr size_t epc = CHUNK_SIZE / sizeof(R);   // physical elems/chunk for THIS R
    ...
    for (size_t i = 0; i < total_pieces; i++) drive_pieces[drive_of[i]].push_back(i);
    ...
    cd::for_each_chunk(d, [&](size_t ci, size_t n, auto it) {
        ...
        for (size_t k = 0; k < pieces[ci]; k++) {
            ...
            for (size_t i = 0; i < take; i++) { out[i] = *it; ++it; }
...
inline std::vector<uint64_t> quickhull_ext_lazy(const chunk_seq& pts,
                                                hpoint l, hpoint r,
                                                size_t budget_elems,
    ...
    auto left_lazy  = cd::lazy_filter(d, [l, mid](const hpoint& p) {
        return area(l, mid, p) > 0; });
    auto right_lazy = cd::lazy_filter(d, [mid, r](const hpoint& p) {
    ...
    std::vector<uint64_t> LR = quickhull_ext_lazy(left_seq,  l,   mid, budget_elems, sp);
    std::vector<uint64_t> RR = quickhull_ext_lazy(right_seq, mid, r,   budget_elems, sp);
    cleanup_prefix(sp + "L");
    cleanup_prefix(sp + "R");
...
inline std::vector<uint64_t> UpperHullLazyFilter(
    ...
    auto above_lazy = cd::lazy_filter(d, [minp, maxp](const hpoint& p) {
        return area(minp, maxp, p) > 0; });
    chunk_seq above = detail::materialize_wide(above_lazy, ap);

    std::vector<uint64_t> between =
        detail::quickhull_ext_lazy(above, minp, maxp, budget_elems, scratch_prefix);
