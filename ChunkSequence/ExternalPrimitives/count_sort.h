...
void count_sort(const D& dseq, size_t num_buckets,
    ...
    // worker.  bucket_allocator is keyed by parlay::worker_id(), a thread_local
    // that silently returns 0 on any thread the scheduler never assigned an id
    // to (deps/parlaylib/parlay/scheduler.h) -- and that keying is inherent to
    // its backing store either way: block_allocator indexes local_lists[id],
    // hazptr_stack indexes acquire_retire's announcements[id]/retired[id].
    // RunIoThread() calls bucket_allocator::free() from Recycle(), so running it
    // on a plain std::thread makes it alias real worker 0's slot -- an
    // unsynchronized data race that corrupts the allocator once enough
    // alloc/free churn hits a bad interleaving (SIGSEGV at n=2^38), and leaks
    // blocks out of the free list (never-reused buffers -> OOM).
    //
    // So the I/O threads run as parlay tasks, exactly as direct_samplesort.h and
    // Peter's scatter_gather.h do.  This is *not* the isolation
    // UnorderedFileWriter / ChunkSequenceReader rely on: their allocators are
    // plain mutex-guarded free lists, which any thread may call.
