 ...
 * over seq -- no ChunkMap write pass, no second co-indexed read.  Same result
 * as count_sort_by_key, but expressed through the delayed layer so the map
  ...
  delayed::for_each_window(dseq, [&](size_t base, size_t w, auto build_chunk) {
    for (size_t b = 0; b < w; b++) {
      ...
      for (size_t k = 0; k < n; k++) {
        ...
        CHECK(j < num_buckets)
            << "count_sort(delayed): bucket id " << j
            << " out of range (num_buckets=" << num_buckets << ")";
...
// lock-free staging buffer; when it fills it is drained under that bucket's
// lock into a single shared CHUNK_SIZE assembly buffer, which is emitted as a
// chunk when full.  Peak DRAM ~= num_workers*num_buckets*STAGE +
// num_buckets*CHUNK_SIZE (the latter matches the serial version's footprint),
// so this stays feasible even when num_buckets grows with n. template<class D>
...
//     for (size_t d = 0; d < num_drives; d++) {
//         filenames[d] = GetFileName(result_prefix, d);
//         int fd = open(filenames[d].c_str(), O_WRONLY | O_CREAT | O_TRUNC,
//         0644); SYSCALL(fd); SYSCALL(close(fd));
//     }
...
//     // Shared per-bucket assembly: one CHUNK_SIZE block being filled, guarded
//     by
...
//     for (size_t b = 0; b < num_buckets; b++) {
//         asm_buf[b] = (T*)aligned_alloc(O_DIRECT_MEMORY_ALIGNMENT,
//         CHUNK_SIZE); CHECK(asm_buf[b] != nullptr) << "count_sort(parallel):
//         assembly alloc failed";
//     }

//     // Drive/offset assignment + per-bucket chunk-list append (both quick,
//     done
//     // once per emitted chunk).  A distinct lock from asm_mu so emitting a
//     full
...
//     // touched only once per STAGE elements, so a larger STAGE means far
//     fewer
//     // lock acquisitions on the hot scatter path (the dominant phase-1 cost
//     at
//     // scale); the trade is W*num_buckets*STAGE of DRAM.  ~32 KiB per stage
//     (8
//     // O_DIRECT blocks) cuts locking 8x versus a single-block stage while
//     keeping
...
//         // Buffers that filled up during this drain; emitted after asm_mu[b]
//         is
...
//                 asm_cnt[b] += take;
//                 off        += take;
//                 if (asm_cnt[b] == ept) {                 // assembly full ->
//                 queue for emit
//                     full_buffers.push_back(asm_buf[b]);
//                     asm_buf[b] = (T*)aligned_alloc(O_DIRECT_MEMORY_ALIGNMENT,
//                     CHUNK_SIZE); CHECK(asm_buf[b] != nullptr) <<
//                     "count_sort(parallel): assembly realloc failed";
//                     asm_cnt[b] = 0;
//                 }
//             }
//             stage_cnt[si] = 0;
//         }
//         // emit_chunk's writer.Push blocks if the writer's queue is full;
//         doing it
//         // here (lock already released) means a slow writer stalls only this
//         // worker, not every other worker draining into the same bucket.
//         Order
//         // within a bucket doesn't matter to either caller (sample_sort
//         re-sorts
