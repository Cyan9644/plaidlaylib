...
void process_inplace(std::vector<chunk_seq>& seqs, Processor processor) {
  ...
  parlay::parallel_for(
      ...
      [&](size_t) {
        ...
        // Rings are built lazily, on the first run that lives on a real
        // device; a heap-backed sequence builds neither.
        ...
        struct Stage {
          ...
          size_t nc = 0, nelem = 0;
          std::vector<Run> runs;
          // How many of `runs` went to the ring; the rest were heap-backed and
          // already copied inline, so there is no completion to reap for them.
          size_t ring_runs = 0;
        };
        ...
        while (submit_read || reap_write) {
          ...
          if (reap_read) {
            for (size_t ri = 0; ri < current.ring_runs; ri++) {
          ...
          if (submit_read) {
            ...
            if (k >= ids.size()) {
              ...
              while (ci < nc) {
                ...
                int fd = plaid::io::Open(first.filename.c_str(), O_RDWR | O_DIRECT);
                SYSCALL(fd);
                if (plaid::io::IsVirtual(fd)) {
                  SYSCALL(plaid::io::Pread(fd, next.buf + start * ept,
                                           read_bytes,
                  ...
                  io_uring_prep_read(sqe, fd, next.buf + start * ept,
                                     read_bytes, (off_t)first.begin_addr);
                  next.ring_runs++;
                }
              ...
              if (next.ring_runs > 0) SYSCALL(io_uring_submit(&read_ring));
            }
          }
          ...
          if (reap_write) {
            for (size_t ri = 0; ri < previous.ring_runs; ri++) {
              struct io_uring_cqe* cqe;
              SYSCALL(io_uring_wait_cqe(&write_ring, &cqe));
              SYSCALL(cqe->res);
              io_uring_cqe_seen(&write_ring, cqe);
            }
            // Closed after the reap (not inside it) so heap-backed runs, which
            // have no completion, still get their fds released.
            for (const Run& run : previous.runs) plaid::io::Close(run.fd);
            free(previous.buf);
          ...
          if (submit_write) {
            ...
            for (const Run& run : current.runs) {
              if (plaid::io::IsVirtual(run.fd)) {
                SYSCALL(plaid::io::Pwrite(
                    run.fd, current.buf + run.start_ci * ept,
            ...
            if (current.ring_runs > 0) SYSCALL(io_uring_submit(&write_ring));
          }
