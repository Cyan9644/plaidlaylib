//this is a documented copy of graduate Peter's filter code for reference
...
FileInfo FilterFile(const FileInfo &in_file, const std::string &out_file, const std::function<bool(const T)> predicate) {
    ...
    while (true) {
        ...
        while (!queue.empty()) {
            ...
            // process buffer
            size_t i = 0;
            while (i < size) {//should be top.size, not size from the block read earlier. that would be fine if we only had one pass but this 
