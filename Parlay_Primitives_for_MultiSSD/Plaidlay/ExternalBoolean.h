#ifndef EXTERNAL_BOOL_H
#define EXTERNAL_BOOL_H
...
template<typename T>
External_Sequence BoolCreate(size_t n, External_Sequence& seq, const std::vector<std::string> &new_filenames, bool val) {
    ...
    //     }
        ...
        for(size_t k = 0; k < buffer_size; k++){
            buffer[i][k] = val; //values past `valid` are padding; `used` records the real count
        }
